	...
	$.fn.lottery = function(options) {
		...
		var initEvent = function() {
			...
			$btn.click(function(e) {
				...
				var prize_index = Math.ceil( Math.random() * (award_len - 1) + 1),
					size = 360 / award_len,
					angle_fix = size / 5,
				...
				$btn.rotate({
					...
					animateTo : angle + options.cycles * 360, 
					callback : function() {
						var index = Math.floor((angle % 360) / (arc * 180 / Math.PI));
						alert(options.texts[index]);
