		...
		return this.each(function(){
			...
			*/
			var mapBox = $('<div id="BDMap" class="BDMap"></div>');			//地图容器Jquery对象
				...
				mapBoxPaent.append($('<p class="map-close-btn">退出</p>'));
		
			if(mapBoxPaent.length>0) var mapH = mapBoxPaent.height();
			if(bigOpen) mapBoxPaent.find(".map-close-btn").css('display','block');
			...
			*/
			...
			handleSuccess = function(position){
				...
				else mapBox.parent().addClass("map-open");
				...
				// 		if(fnClose) fnClose();
				// 	}else {
				// 		mapBoxPaent.removeClass("map-open");
				// 		if(fnClose) fnClose();
				// 	}
			...
			*/
			...
			// 	else{
			// 		mapBoxPaent.toggleClass("map-open");
			// 	}
			// 	if(!mapBoxPaent.hasClass('open')||!mapBoxPaent.hasClass("map-open")){
			// 		if(fnClose) fnClose();	
			// 	}
			// });
			
			$('.map-close-btn').on('click',function(){
				mapBoxPaent.removeClass("map-open open");
				if(fnClose) fnClose();
			});
			...
			*/
			...
			function loadfunction() { 
				...
				if($('.BDC').length<=0){
					...
					var mapScale = phoneScale = 1;
					var height = $(window).height();

					var style_map =
							 ".bdmap.open,.bdmap.map-open {height:100%;width:100%;background:#fff;}"+
							 ".bdmap img {max-width:initial!important;}"+
							 ".bdmap .tit { position:absolute; left:0; bottom:0; height:70px; width:100%; overflow: hidden; background:rgba(0,0,0,0.5); }"+
							 ".bdmap .tit p { margin-right:100px; }"+
							 ".bdmap .tit p a { position:relative; display:block; font-size:24px; color:#fff; height:70px; line-height:70px; padding-left:70px; }"+
							 ".bdmap .tit p a span { position:absolute; left:15px; top:15px; display:inline-block; width:40px;height:40px; }"+
							 ".bdmap .tit .close_map { display:none; position: absolute; bottom: 15px; right: 20px; width: 40px; height: 40px; margin-right:0; cursor:pointer; background-position: -100px -73px; }"+
							 ".bdmap .map-close-btn{position:absolute;top:10px;left:10px;display:none;width:80px;box-shadow:0 0 2px rgba(0,0,0,0.6) inset, 0 0 2px rgba(0,0,0,0.6);height:80px;border-radius:80%;color:#fff;background:rgba(230,45,36,0.8);text-align:center;line-height:80px;font-size:26px; font-weight:bold;cursor:pointer;}"+
							 ".bdmap.open .map-close-btn{display:block;}"+
							 ".bdmap.mamap-openmap-close-btn{display:block;}"+
							 "#BDMap {transform:scale("+mapScale+");-webkit-transform:scale("+mapScale+");}"+
							 "#BDMap {width:100%;height:100%;}"+
							 "#BDMap img{width:auto;height:auto;}"+
							 ".bdmap.open .transitBtn{display:block;}"+
							 ".bdmap.map-open .transitBtn{display:block;}"+
