	...
	$.fn.progress = function(options) {
		...
		var runTime = function() {
			...
			vthis.each(function() {
				...
				var showTime = function(rT, showTpl, showXTpl) {
					...
					nthis.attr('data-laststr', showXTpl.replace(/%S%/, s_).replace(/%M%/, m_).replace(/%H%/, h_).replace(/%D%/, d_));
				};
				
				var state = 0;
				if (sT > 0) {// 即将开始
					state = 1;
					showTime(sT, s.stpl, s.xtpl);
				} else if (eT > 0) {//正在进行
					state = 2;
