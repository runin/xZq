;(function($) {
    H.yao = {
        ...
        textMath: function() {//随机文案
            ...
            H.yao.$textb.text("就这样与红包擦肩而过了，继续加油哦！");
        },
        ...
        beforeShowCountdown: function(pra) {
            ...
            me.countdown_domShow(beginTimeStr,"距摇奖开启还有",1);
        ...
        countdown_domShow: function(time, word,none){
            var me = H.yao,
            timeLong = timestamp(time);
            timeLong += me.dec;
            $('.items-count').removeClass('none');
            if(none){
                $('.detail-countdown').attr('etime',timeLong).addClass('none');
                $(".countdown-tip").html("本期活动未开始,请等待主持人提示").css("font-size","13px");
            }else{
                $('.detail-countdown').attr('etime',timeLong);
                $(".countdown-tip").html(word).css("font-size","18px");
            }
            //$('.detail-countdown').attr('etime',timeLong);
        ...
        change: function(){
            ...
            $('.items-count').addClass('none');
            $('.loading').removeClass('none');
            $(".countdown").removeClass("none").html('本期活动已结束，请退出当前链接').css("font-size","13px");
