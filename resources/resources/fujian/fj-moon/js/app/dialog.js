...
//								._('<a href="#" class="btn-know close" data-collect="true" data-collect-flag="ruledialog-closebtn" data-collect-desc="规则弹层-关闭按钮">我知道了</a>')
		...
		rank: {
			...
			selfurank:function(data){
				var t = simpleTpl(),h;
				if(!headimgurl){
						h = 'images/danmu-head.jpg';
					}else{
						h = headimgurl+'/'+yao_avatar_size
				}
				t._('<span class="r-avatar"><img src="'+ h+'" /></span>')
			...
			event: function() {
				...
				$('#btn-award').click(function(e) {
					...
					if(H.dialog.lottery.pt == 4){
						...
						return;
					//实物奖品
					}else if(H.dialog.lottery.pt == 1||H.dialog.lottery.pt == 2){
