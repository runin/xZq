(function($){
    H.index = {
                        ...
                        for(var i = 0;(i<=data.items.length - 1);i++){
                            ...
                            if((serverT>st)&&(serverT<et)){
                                ...
                                for(var a = 0; a < data.items[i].pitems.length;a++){
                                    ...
                                    //infoData += '<div class="card-bottom"><span class="card-name"></span><a href="#" class="praise-btn" onclick="H.index.praise(this)" puid="' + data.items[i].pitems[a].pid + '" guid="' + data.items[i].guid + '" data-collect="true" data-collect-flag="vote-sign" data-collect-desc="点赞按钮"></a>';
                                    ...
                                    //infoData += '<p class="card-info"></p></div></div>';
        ...
        count_down: function() {
            ...
            me.$lotteryCountdown.find('.detail-countdown').each(function() {
                $(this).countDown({
                    etpl: '<span class="fetal-H1">%H%' + '</span>时<span class="fetal-H1">' + '%M%' + '</span>分<span class="fetal-H1">' + '%S%' + '</span>秒', // 还有...结束
                    stpl: '<span class="fetal-H1">%H%' + '</span>时<span class="fetal-H1">' + '%M%' + '</span>分<span class="fetal-H1">' + '%S%' + '</span>秒', // 还有...开始
                    sdtpl: '',
                    otpl: '',
                    otCallback: function() {
                        if(me.canJump){
    ...
    W.callbackLinesDiyInfoHandler = function(data){
    	if(data.code == 0){
    		$("body").css({
                "background": data.gitems && data.gitems[0].is?data.gitems[0].is:"images/index-bg.png",
