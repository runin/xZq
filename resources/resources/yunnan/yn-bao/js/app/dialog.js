(function($) {
	H.dialog = {
		...
		rule: {
			...
			tpl: function() {
				...
				t._('<section class="modal" id="rule-dialog">')
					._('<a href="#" class="btn-close" data-collect="true" data-collect-flag="ah-bao-ruledialog-closebtn" data-collect-desc="规则弹层-关闭按钮"></a>')
			...
			tpl: function() {
				...
				t._('<section class="modal" id="rank-dialog">')
					...
					._('<div class="dialog rank-dialog">')
						._('<h2>积分排行榜</h2>')
						._('<p>全剧终积分前三名可依次获得<br>iphone 6一台、ipad mini2一台、ipad nano6 一台</p>')
		...
		lottery: {
			...
			update: function(data) {
				...
				// 换个姿势文案隐藏
				this.$dialog.find('.guajiang-tips').addClass('none');
			...
			check: function() {
				...
				if (me.pt == 1 && (address.length < 5 || address.length > 60 || address.length == 60)) {
					alert('地址长度为5~60个字符哦');
					$address.focus();
					me.enable();
					return false;
			...
			tpl: function() {
				...
				t._('<section class="modal" id="lottery-dialog">')
					._('<a href="#" class="btn-close" data-collect="true" data-collect-flag="ah-bao-lotterydialog-closebtn" data-collect-desc="抽奖弹层-关闭按钮"></a>')
					._('<div class="dialog lottery-dialog">')
						._('<div class="dialog-inner">')
							._('<h2>恭喜你，答对了<div class="guajiang-tips">换个好姿势刮奖吧！</div></h2>')
							._('<div class="content">')
								...
								._('<div class="back none">')
									...
									._('<div class="award-win">')
										...
										._('<div class="contact">')
											...
											._('<p><input type="number" class="mobile" placeholder="手机号码" /></p>')
	...
	// 		});
	// 		$cover.addClass('zshow');
	// 		if (data.code == 0) {
	// 			lottery.init(data.pi);
	// 		} else {
	// 			lottery.init();
	// 		}
	// 		lottery.init(data.pi);
