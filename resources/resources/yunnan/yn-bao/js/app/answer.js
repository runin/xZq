(function($) {
	H.answer = {
					...
					t._('<li data-qcode="'+ qitems[i].qcode +'" data-ruid="'+ qitems[i].ruid +'" data-stime="'+ timestamp(qitems[i].qst) +'" data-etime="'+ timestamp(qitems[i].qet) +'" id="question-'+ qitems[i].quid +'" data-quid="'+ qitems[i].quid +'">')
						._('<dl>')
							...
							for (var j = 0, jlen = aitems.length; j < jlen; j ++) {
								t._('<dd><a href="#" class="q-item" data-auid="'+ aitems[j].auid +'" data-collect="true" data-collect-flag="yn-bao-answer-answer" data-collect-desc="答题页面-答案按钮">'+ aitems[j].at +'</a></dd>');
							}
		...
		event: function() {
			...
			this.$question.delegate('.q-item', 'click', function(e) {
				...
				// alert('调接口callbackYiguanAnswer');
				getResult('yiguan/answer', {
					yoi: openid,
					auid: $(this).attr('data-auid')
				}, 'callbackYiguanAnswer', true, null, false);
		...
		answered: function(data) {
			...
			if (data.rs === 1) {
				// alert('调接口callbackYiguanLottery');
				getResult('yiguan/lottery', {
					yoi: openid
				}, 'callbackYiguanLottery', true, null, false);
			}
		},
	...
	W.callbackYiguanAnswer = function(data) {
		// alert('进入回调 callbackYiguanAnswer');
		if (data.code == 0) {
