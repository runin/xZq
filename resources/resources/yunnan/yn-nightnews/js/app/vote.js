(function($){
    H.vote = {
        ...
        checkGuide: function() {
            var me = this;
            if ($.fn.cookie(mpappid + '_' + openid + '_guide') == 1) {
                $('#J_guide').addClass('none');
            } else {
                $('#J_guide').removeClass('none');
                $.fn.cookie(mpappid + '_' + openid + '_guide', 1, me.expires_in);
            }
        },
        event: function(){
            var me = this;
            $('#avatar').tap(function(e) {
                e.preventDefault();
    ...
    W.callbackIntegralRankSelfRoundHandler = function(data) {
        var me = H.detail;
        if (data.result) {
            $('.btn-coin span').text(data.in * 1 + '积分');
        }
    };
