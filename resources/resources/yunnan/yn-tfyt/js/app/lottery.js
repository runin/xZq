    ...
    H.lottery = {
        ...
        event: function() {
            ...
            $('body').delegate('#test', 'click', function(e) {
                ...
                me.lotteryTime = 1;
                me.shake_listener();
            }).delegate('#big', 'click', function(e) {
                e.preventDefault();
                if ($('.marquee').hasClass('key')) {
                    $('.marquee').removeClass('key');
                } else {
                    $('.marquee').addClass('key');
                }
            ...
            }).delegate('#btn-talk', 'click', function(e) {
                e.preventDefault();
                if(!$(this).hasClass('requesting')){
                    $(this).addClass('requesting');
                    toUrl('talk.html');
        ...
        red_record: function() {
            ...
            // $.ajax({
            //     type: "GET",
            //     url: "http://dynamic.m.tuniu.com/event/lottery/util/getPrizedListAjax",
            //     data: {
            //         "actId":456, //活动ID
            //         "mark": "cwyyymg", //活动标识
            //         "isBig": 0, //是否大奖
            ...
            //                     var username = (list[i].tel || "匿名用户");
            //                     con += "<li><span>" + username + "中了" + list[i].prizeName + "</span></li>";
            //                 }
        ...
        big_record: function() {
            ...
            //             var list = data.data;
            //             if(list && list.length > 0){
            //                 var username = (list[0].tel || "匿名用户");
            //                 $('.point span').html(username + "中了" + list[0].prizeName);
            //                 $('.marquee').addClass('key');
            //             } else {
            //                 $('.marquee').removeClass('key');
            //             }
        ...
        initComponent: function(){
            var me = this, recordDelay = Math.ceil(3000*Math.random() + 3000), bigrecordDelay = Math.ceil(10000*Math.random() + 10000), allRecordTime = Math.ceil(20000*Math.random() + 600000);
            setTimeout(function(){ me.red_record(); }, recordDelay);
            setInterval(function(){ me.red_record(); }, allRecordTime);
            // setInterval(function(){ me.big_record(); }, bigrecordDelay);
        },
    ...
    W.commonApiPromotionHandler = function(data){
        if (data.code == 0 && data.url) {
            ...
            $('#tttj').click(function(e) {
                e.preventDefault();
                if ($('#tttj').hasClass('requesting')) {
                    return;
                }
                $('#tttj').addClass('requesting');
