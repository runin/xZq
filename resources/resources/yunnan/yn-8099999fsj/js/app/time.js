            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data.t){
                        ...
                        H.time.dec = nowTimeStemp - data.t*1;
                    }
                }
            });
        },
            ...
            $.ajax({
                ...
                success : function(data) {
                    if(!data.result || data.flow == 1){
                        if(H.time.repeat_load){
                            H.time.repeat_load = false;
                            setTimeout(function(){
                                H.time.current_time();
                            },500);
                        }
                    }else{
                        H.time.nowTime = timeTransform(data.sctm);
                        var nowTimeStemp = new Date().getTime();
                        H.time.dec = nowTimeStemp - data.sctm;
                        H.time.roundData = data;
                        H.time.currentPrizeAct(data);
                    }
                },
