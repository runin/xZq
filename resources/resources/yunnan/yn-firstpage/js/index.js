    ...
    N.module("firstPage", function () {
        ...
        this.initParam = function () {
            ...
            function show_message(index) {
                if (that.items[index]) {
                    ...
                    var bbtn = $("<div class='bottom_btn'>预 约</div>");

                    if (that.items[index].iscanPre && that.items[index].reserveId) {
                        if ($.fn.cookie("reserve_code" + yao_tv_id + that.items[index].reserveId)) {
                            bbtn.text("已预约");
                            bbtn.addClass("gay");
                        } else {
                            bbtn.attr("reserveId", that.items[index].reserveId); //预约
                            bbtn.attr("date", that.items[index].date); //预约
                        }
        ...
        this.appendData = function (items) { //构造数据
            ...
            for (var i = 0, len = items.length; i < len; i++) {
                ...
                sitem.find(".jumpa").attr("href", items[i].url); //跳转
