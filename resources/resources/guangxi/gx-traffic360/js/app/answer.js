	...
	H.answer = {
				...
				t._('<li data-qriu="'+ qitems[i].qriu +'" data-qcode="'+ qcode +'" data-ruid="'+ ruid +'" data-stime="'+ timestamp(qitems[i].qst) +'" data-etime="'+ timestamp(qitems[i].qet) +'" id="question-'+ qitems[i].quid +'" data-quid="'+ qitems[i].quid +'" >')
					...
					._('<div class="dorA">')
						...
						for (var j = 0, jlen = aitems.length; j < jlen; j ++) {
							t._('<a class="q-item" href="#" id="answer-'+ aitems[j].auid +'" data-auid="'+ aitems[j].auid +'" data-collect="true" data-collect-flag="gx-traffic360-answer-item" data-collect-desc="答题页-答案按钮"><i></i>'+ aitems[j].at +'</a>')
						}
					t._('</div>')
		...
		progress: function(data) {
			...
			this.$question.find('li').each(function() {
				...
				$me.progress({
					cTime: data,
					stpl : '<p class="count"><span>%H%</span>时<span>%M%</span>分<span>%S%</span>秒</p>',
					callback: function(state) {
						if(me.TIMETRUE_CLS) {
							...
							//所有答题结束
								...
								me.$timebox.html("本期活动已结束，请等待下期!").addClass("none");
								$('.answer-box').addClass('ended');
								$('.answer-box .show-box').addClass('hidden');
							...
							if ($started.length > 0) {
								...
								if ($starting.length > 0) {
									...
									}else if(H.answer.collectAnswer){
										$("#show-"+H.answer.collectAnswer).addClass("selected");
										$("#show-"+$prev.attr("data-qriu")).addClass("right");
									}else{
