(function($) {
    ...
    W.callbackSignIsHandler = function(data) {
        ...
        if (data.result) {
            ...
            if ($('#sign-' + me.signList[me.signListFlag]).prev().hasClass('active')) {
                $('#sign-' + me.signList[me.signListFlag]).find('i').text($('#sign-' + me.signList[me.signListFlag]).attr('data-iv') + '金币');
            } else {
                $('#sign-' + me.signList[me.signListFlag]).find('i').text($('#sign-' + me.signList[me.signListFlag]).attr('data-bv') + '金币');
            }
        }
    ...
    W.callbackSignSignedHandler = function(data) {
        var me = H.sign;
        if (data.code == 0) {
            $('#sign-' + me.nowSignFlag).addClass('active').find('i').text(data.signVal + '金币');
        }
