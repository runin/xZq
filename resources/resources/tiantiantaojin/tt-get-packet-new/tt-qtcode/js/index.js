$(function () {
    ...
    N.module("pre_mainPage", function () {
        ...
        this.loadData = function () {//加载列表数据 
            var that = this;
            N.loadData({ url: business_url + "redpack/list", callbackRedPackListHandler: function (data) {
                if (data.code == 0) {
        ...
        this.appendData = function (data) {//初始化列表
            ...
            arr.push('<a class="pre_link" href="#" data-item="xx" data-qtcode=""></a>');
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                item.find(".play_type").text(data[i].type == 1 ? "参与时间" : "参与时间");
                item.find(".pre_link").attr("href", "javascript:void(0)");
                item.find(".pre_link").attr("data-item",i);
                item.find(".pre_link").attr("data-qtcode",data[i].dimensionUrl?data[i].dimensionUrl:"");
        ...
        this.initPreBtnEvent = function () {//初始化列表事件
            ...
            $(".pre_link").click(function (e) {
                if ($(e.target).hasClass("pre_link")) {
                    ...
                    var eventDesc = $(this).attr("data-collect-desc");
                    var eventId = $(this).attr("data-collect-flag");
                    var qtCode = $(this).attr("data-qtcode");
                    ...
                    {
                        window.location.href = "./qtcode.html?itemId="+itemId;
                    }
                    // 
        ...
        this.userlog = {//用户日记
            add: function (openid, eventDesc, eventId) {
                N.loadData({ url: business_url + "user/operate/log/add", callbackAddUserOperateLogHandler: function (data) {
                }, data: {
                    appId: busiAppId,
                    openId: openid,
