(function($) {
        ...
        H.market = {
                ...
                exchangeInfor:function()
                {
                    getResult("goods/gourl",{appId:busiAppId}, "goUrlCallBackHandler", false);
                },
                event: function() {
                    ...
                    $("body").delegate(".gold-deduct","click", function(e) {
                            var me = this;
                            e.preventDefault();
                            goodflag = false;
                            flag = 2;
            ...
            specialDate: function(data) {
                ...
                $(".top-image-nav2  p").text("点此兑换一元夺宝“满10减1”代金券。一元夺宝全场商品单次支付满10元宝均可参与抵现优惠活动。");
            ...
            update: function(data) {
                ...
                for (i = 0; i < length; i++) {
                    t._('<li onclick=exchangePrize(this,' + data[i].goodsId + ')  red_state="' + data[i].isred + '">')
                        ...
                        ._('</a>')
                        ._('<div class="ex-change-prize">' + data[i].goodsValue + '金币点击兑换</div>')
                        ._('</li>');
                }
    ...
    W.goUrlCallBackHandler = function(data)
    {
        if(data.result)
        {
            $(".btn-rule").removeClass("none");
             // 活动规则
