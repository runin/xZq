; (function (w) {
    ...
    w.openid = $.fn.cookie(mpappid + '_openid');//olyOrjirhJHdxVRtHwnUKsP5tKvM//"olyOrjltoEUT2OJjzKkCJAmCeMEg" //"olyOrju6iaGdrFtP24zvJaioKJYs11"; $.fn.cookie(mpappid + '_openid');
    ...
    w.Authorize = function (o) {
        ...
        this.redirect_uri = domain_url + "api/mp/auth/auth_userinfo";//redirect_uri
        this.callBackPage = o && o.callBackPage||"";//授权之后的回调页面
        this.param = "";//微信的参数
    }
    Authorize.prototype.authorize = function (fn) {
      window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + this.mpappid + "&redirect_uri=" + encodeURIComponent(this.redirect_uri+"?callBackPage="+this.callBackPage+"&authAppId="+this.mpappid+"&busiAppId="+busiAppId)+ "&response_type=code&scope=" + this.scope +  "&state="+state+"#wechat_redirect";
      if(fn){
         fn();
      }
    };
   ...
   new Authorize({callBackPage:"reduserguide.html"}).init();
})(window);
