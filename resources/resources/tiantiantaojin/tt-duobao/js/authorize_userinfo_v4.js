...
;(function(w) {
    ...
    w.timestamp = $.fn.cookie(mpappid + '_timestamp');
    w.shareOpenid = $.fn.cookie(mpappid + '_shareOpenid');
    if (getQueryString("timestamp")) {
        timestamp = $.fn.cookie(mpappid + '_timestamp', getQueryString("timestamp"), { expires: exp });
        shareOpenid = $.fn.cookie(mpappid + '_shareOpenid', getQueryString("shareOpenid"), { expires: exp });
    }
    //获取分享用户加密的openid
    ...
    Authorize.prototype.authorize = function(fn) {
        ...
        window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + this.mpappid + "&redirect_uri=" + encodeURIComponent(this.redirect_uri) + "&response_type=code&scope=" + this.scope + "&state=" + serviceNo4 + "#wechat_redirect";
        if (fn) {
            fn();
        }
    };
    ...
    var callBackPageUrl = "redshare.html";
    ...
    var tt_authorize = function(fn) {
        authorize_count++;
        window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + busiAppId + "&redirect_uri=" + encodeURIComponent(redirect_url + "?callBackPage=" + callBackPageUrl + "&openId=" + openid + "&authAppId=" + busiAppId) + "&response_type=code&scope=" + tttj_scope + "&state=" + state + "#wechat_redirect";
    };
