...
(function(w) {
    ...
    Authorize.prototype.authorize = function(fn) {
        ...
        window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + this.mpappid + "&redirect_uri=" + encodeURIComponent(this.redirect_uri + "?param=" + this.param) + "&response_type=code&scope=" + this.scope + "&state=" + seckillNo + "#wechat_redirect";
        if (fn) {
            fn();
        }
    };
    ...
    var callBackPageUrl = "html/seckill/seckillview.html";
    var check_tt_authorize = function() {
        if (!/micromessenger/i.test(navigator.userAgent)) {
            window.location.href = 'wx.html';
            return false;
        }
        tt_param = encodeURIComponent(JSON.stringify({ data_uuid: getQueryString("data_uuid") }));
        if (!openid) {
            return
