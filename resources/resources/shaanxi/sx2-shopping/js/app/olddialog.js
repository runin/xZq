(function($) {
	H.dialog = {
			...
			tpl: function() {
				...
				t._('<section class="modal modal-guide" id="guide-dialog">')
					._('<div class="dialog guide-dialog relocated">')
						...
						._('<a href="#" class="btn btn-try" data-collect="true" data-collect-flag="guide-trybtn" data-collect-desc="引导弹层-关闭按钮">马上去试试</a></div>')
					._('</div>')
				._('</section>');
				return t.toString();
			}
		...
		lottery: {
			...
			open: function(data) {
				...
				if(data.pt == 7){
					...
					document.getElementById('aw').onload = function () {
						//inheight = ($(".contact").height() + $(".awardwin-tips").height() + $(".award-img").height() + 35) + (winH * 0.15);
						//if(inheight >= winH){
						//	$(".award-img img").css("height","60px");
						//	inheight = ($(".contact").height() + 95) + (winH * 0.2);
						//}
						//$(".lottery-dialog").css({
						//	'top': (winH - inheight) * 0.5 + 'px',
						//	'height': inheight+'px'
						//});
					};
				}else if(data.pt == 4){
			...
			event: function() {
				...
				this.$dialog.find('.btn-award').click(function(e) {
					...
					if(H.dialog.lottery.pt == 7){
						...
						setTimeout(function () {
							shownewLoading();
							window.location.href = H.dialog.ru;
						},1000);
				...
				if (data != null && data != '') {
					if (data.result) {
						if (data.pt === 1) { //实物奖品
							...
							$(".award-img").css("margin","auto");
							//$(".award-win").css({"margin":"auto","background": "url(./images/newaward-bg.jpg) no-repeat 100% 100%!important","background-size": "100% 100%!important"});
							...
							//this.$dialog.find('.btn-share').css({"background":"url(./images/award-btn.png) 0 0 no-repeat","background-size":"100%"});
							$(".award-bg").css({"background": "url(./images/award-bg.png) no-repeat 100% 100%!important","background-size": "100% 100% !important"})
							$(".award-img").css("margin","auto");
							//$(".award-win").css({"margin":"auto","background": "url(./images/newaward-bg.jpg) no-repeat 100% 100%!important","background-size": "100% 100%!important"});
							...
							$(".red").css("top",(($(".back").height() - ($(window).width() * 0.86)) * 0.5) + "px").removeClass('none').on("click", function () {
								...
								$(".award-img").css("margin","18% auto 18%");
								$(".awardwin-tips").css("margin","20% auto 0");
								if(($(window).height()<520)){
									$(".award-img").css("margin","15% auto 15%");
									$(".awardwin-tips").css("margin","15% auto 0");
								}
								if($(window).height()<440){
									$(".award-img").css("margin","8% auto 5%");
									$(".awardwin-tips").css("margin","10% auto 0");
								}
								$(".award-bg").css({"background": "url(./images/award-bg.png) no-repeat 100% 100%!important","background-size": "100% 100% !important"})
								$(".red-p").addClass("none");
								$(".red-head").addClass("none");
							});
							$(".red-p").text(data.pd).removeClass("none");
							$(".red-head").attr("src",data.qc).removeClass("none");
						} else {
			...
			check: function() {
				if(H.dialog.lottery.pt == 4 || H.dialog.lottery.pt == 7 || H.dialog.lottery.pt == 9){
			...
			tpl: function() {
				...
				t._('<section class="modal" id="lottery-dialog">')
					...
					._('<p class="red-p none">恭喜获得红包，快打开吧！</p>')
					._('<img class="red-head none" src="images/award-red-head.png" />')
