...
function callbackStationMallDetail(data) {
	if (data.code == 0) {
		var t = simpleTpl();
		$('#cover').html('<img src="'+ domain_url + data.ib +'" />');
		$('#info').find('h2').text(data.n || '').end()
				  .find('.jf strong').text(data.ip || 0).end()
				  .find('.price strong').text(Math.round((data.mp || 0)) / 100).end()
				  .find('.kc strong').text(data.ic == -1 ? '充足' : (data.ic + data.ut)).end()
				  .data('tip', data.tip || '');
		
		$('#detail-list').find('.xq').html('<p>'+ data.d +'</p>').end()
						 .find('.cs').html('<p>'+ (data.param || '') +'</p>');
	}
}
