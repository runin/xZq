		...
		return this.each(function(){
			...
			*/
			var mapBox = $('<div id="BDMap" class="BDMap"></div>');			//地图容器Jquery对象
				...
				mapBoxPaent.append($('<p class="map_close_btn" data-collect="true" data-collect-flag="beauty-map-close" data-collect-desc="退出地图">退出</p>'));
			...
			*/
			...
			function loadfunction() { 
				...
				if($('.BDC').length<=0){
					...
					var style_map =
							 ".bdmap.open,.bdmap.mapOpen {height:100%;width:100%;background:#fff;}"+
							 ...
							 ".bdmap .map_close_btn{position:absolute;top:10px;left:10px;display:none;width:80px;box-shadow:0 0 2px rgba(0,0,0,0.6) inset, 0 0 2px rgba(0,0,0,0.6);height:80px;border-radius:80%;color:#fff;background:rgba(230,45,36,0.8);text-align:center;line-height:80px;font-size:26px; font-weight:bold;cursor:pointer;}"+
							 ".bdmap.open .map_close_btn{display:block;}"+
							 ".bdmap.mapOpen .map_close_btn{display:block;}"+
							 ...
							 ".bdmap.mapOpen .transitBtn{display:block;}"+
