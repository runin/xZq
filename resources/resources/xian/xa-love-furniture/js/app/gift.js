(function($){
	H.gift = {
		...
		init : function(){
			...
			this.scroll();
			$('.main').css({
				'min-height': $(window).height()-$(window).height()*0.35
			});
		},
		event : function(){
		...
		getList : function(){
			if(openid){
				...
				if(!getQueryString('length') && this.first){
					...
					t._('</ul><img class="code" src="'+ $.fn.cookie("pqi-"+ openid) +'"><div class="code-red"></div>');
					//t._('</ul><img class="code" src="'+ $.fn.cookie("pqi-"+ openid) +'"><div class="code-red">长按二维码 即可关注我们</div>');
					$('#gift-div').append(t.toString()).closest('.list').removeClass('none');
