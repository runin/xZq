...
//  'use strict';
    ...
    function init() {
        ...
        if (options.arrow) {
            ...
            $($pageArr[0]).find('.parallax-arrow').remove();
        }
    }
            ...
            if (options.swipeAnim === 'default') {
                ...
                if (Math.abs(endPos-startPos) <= 50 && endPos >= startPos && options.drag) {
                	...
                	if(isHead() && firstHead){
                		animatePage(curPage, 'keep-forward');
                        direction = 'stay';
                	}else if(isHead() && !firstHead){
                		animatePage(pageCount-1);
                		animatePage(pageCount-1, 'backward');
                	}else{
                		animatePage(curPage-1, 'backward');
                	}
                    direction = 'backward';
    ...
    function dragToMove() {
        ...
        if (options.swipeAnim === 'default') {
            ...
            var temp      =  endPos - startPos , $prevPage , $nextPage;
            if(curPage === 0 && !firstHead){
                $prevPage = $($pageArr[pageCount-1]);
                $nextPage = $($pageArr[curPage+1]);
            }else if(curPage === pageCount-1){
    ...
    function isTail() {
        if (options.direction === 'horizontal') {
            if ((endPos <= startPos && curPage === pageCount-1)) {
            	if(firstHead){
            		firstHead = false;
            	}
                return true;
