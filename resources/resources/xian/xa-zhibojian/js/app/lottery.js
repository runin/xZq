(function($) {
    H.lottery = {
        ...
        shake_listener: function() {
	          ...
	          recordUserOperate(openid, "西安直播间摇一摇", "tv-xian-zhibojian-shake");
	          recordUserPage(openid, "西安直播间摇一摇", 0);
	          H.lottery.times++;
		...
		count_down : function() {
			$('.detail-countdown').each(function() {
				var $me = $(this);
				$(this).countDown({
					etpl : '<p>%H%</p>' + '<label class="dian">时</label>' + '<p>%M%</p>' + '<label class="dian">分</label>' + '<p>%S%</p>'+'<label class="dian">秒</label>', // 还有...结束
					stpl : '<p>%H%</p>' + '<label class="dian">时</label>' + '<p>%M%</p>' + '<label class="dian">分</label>' + '<p>%S%</p>'+'<label class="dian">秒</label>', // 还有...开始
					sdtpl : '',
					otpl : '',
					otCallback : function() {
						if(!H.lottery.isTimeOver){
