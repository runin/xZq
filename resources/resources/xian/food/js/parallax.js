...
!function($) {
	...
	function init() {
		...
		if (options.loading) {
			$('.wrapper')
					.append(
							'<div class="parallax-loading"><i class="ui-loading ui-loading-white"></i></div>');
		} else {
			// 允许触摸滑动
			movePrevent = false;
		}
		...
		$pages.css('width', pageWidth * $pageArr.length) : $pages.css('height',
				pageHeight * $pageArr.length);

		if (options.swipeAnim === 'cover') { // 重置 page 的宽高(因为这两个效果与 default
												// 实现方式截然不同)
			$pages.css({
				'width' : pageWidth,
				'height' : pageHeight
			});
			$pageArr[0].style.display = 'block'; // 不能通过 css 来定义，不然在 Android
													// 和 iOS 下会有 bug
		}
	...
	function onStart(e) {
		...
		options.direction === 'horizontal' ? startPos = e.pageX
				: startPos = e.pageY;

		if (options.swipeAnim === 'default') {
			$pages.addClass('drag'); // 阻止过渡效果

			offset = $pages.css("-webkit-transform").replace("matrix(", "")
					.replace(")", "").split(",");

			options.direction === 'horizontal' ? offset = parseInt(offset[4])
	...
	function onMove(e) {
		...
		event.preventDefault();
		options.direction === 'horizontal' ? endPos = e.pageX
				: endPos = e.pageY;
		...
		if (movePrevent === true || stage !== 2) {
			...
			if (options.swipeAnim === 'default' && !isHeadOrTail()) {
				...
				if (Math.abs(endPos - startPos) <= 50) {
					...
					animatePage(curPage + 1);
					direction = 'forward';
					if ($.inArray(curPage + 1, pageArr) == -1
							&& curPage + 1 != pageCount && curPage + 2 != pageCount) {
						add_loadding(curPage+1);
						pageArr.push(curPage + 1);
						bind_move();
					}
				...
				if (Math.abs(endPos - startPos) <= 50 && endPos >= startPos
					...
					direction = 'stay';
				} else if (Math.abs(endPos - startPos) <= 50
						&& endPos < startPos && options.drag) {
			...
			if (options.indicator) {
				$(
						$(
								'.parallax-h-indicator ul li, .parallax-v-indicator ul li')
								.removeClass('current').get(curPage)).addClass(
						'current');
			}
			stage = 3;
		}
	...
	function dragToMove() {
		...
		if (options.swipeAnim === 'default') {
			...
			options.direction === 'horizontal' ? $pages.css(
					"-webkit-transform", "matrix(1, 0, 0, 1, " + temp + ", 0)")
					: $pages.css("-webkit-transform", "matrix(1, 0, 0, 1, 0, "
							+ temp + ")");
		}
			...
			if (options.direction === 'horizontal' && endPos >= startPos) {
				$prevPage.css({
					'z-index' : 2,
					'display' : 'block',
					'-webkit-transform' : 'translateX(' + (temp - pageWidth)
							+ 'px)'
				})
				...
				$nextPage.css({
					'z-index' : 2,
					'display' : 'block',
					'-webkit-transform' : 'translateX(' + (pageWidth + temp)
							+ 'px)'
				})
				...
				$prevPage.css({
					'z-index' : 2,
					'display' : 'block',
					'-webkit-transform' : 'translateY(' + (temp - pageHeight)
							+ 'px)'
				})
			} else if (options.direction === 'vertical' && endPos < startPos) {
				$nextPage.css({
					'z-index' : 2,
					'display' : 'block',
					'-webkit-transform' : 'translateY(' + (pageHeight + temp)
							+ 'px)'
				})
			}
		}
	...
	function animatePage(newPage, action) {
		...
		if (options.swipeAnim === 'default') {
			...
			var newOffset = 0;
			options.direction === 'horizontal' ? newOffset = newPage
					* (-pageWidth) : newOffset = newPage * (-pageHeight);

			options.direction === 'horizontal' ? $pages.css({
				'-webkit-transform' : 'matrix(1, 0, 0, 1, ' + newOffset
						+ ', 0)'
			}) : $pages.css({
			...
			if (action === 'keep-backward' && options.drag) {
				...
				options.direction === 'horizontal' ? $($pageArr[curPage - 1])
						.css({
				...
				$pageArr.removeClass('drag');
				$($pageArr[curPage - 1]).addClass('back'); // 纯粹为了在动画结束后隐藏，不涉及
															// CSS 中定义的动画
	...
	function animShow() {
		...
		$('.current [data-animation]')
				.each(
						function(index, element) {
							var $element = $(element), $animation = $element
									.attr('data-animation'), $duration = $element
									.attr('data-duration') || 500, $timfunc = $element
									.attr('data-timing-function')
									|| 'ease', $delay = $element
									.attr('data-delay') ? $element
									.attr('data-delay') : 0;
		...
		if (!isMoveDisabled()) {
			...
			function() {
				...
				if (direction !== 'stay') {
					...
					$($pageArr.removeClass('current').get(curPage)).addClass(
							'current');
			...
			.on(
					...
					function() {
						...
						if (options.loading) {
							...
							movePrevent = false;
							$($pageArr[curPage]).addClass('current');
							options.onchange(curPage, $pageArr[curPage],
									direction);
							animShow();
						}
						...
						if (options.indicator) {
							...
							var temp = options.direction === 'horizontal' ? 'parallax-h-indicator'
						...
						if (options.arrow) {
							$pageArr
									.append('<span class="parallax-arrow"></span>');
							$($pageArr[pageCount - 1]).find('.parallax-arrow')
									.remove();
						}
					});
