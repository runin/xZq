(function($) {
    H.lottery = {
        ...
        event: function() {
            ...
            $(".jmd ").on('tap',function(e){
                e.preventDefault();
                me.btn_animate($(this));
                toUrl('program.html?type=lottery');
            });
            $(".ljgz").on('tap',function(e){
                e.preventDefault();
                me.btn_animate($(this));
                H.dialog.rule.open();
            })
        ...
        showBigawards_tip: function(beginTimeStr){
            var $big_award = $("#big-award"),
                flag = true;
                $.each(awards_time, function(j,jtem){
                    if(flag){
                        if(jtem === beginTimeStr){
                            $big_award.removeClass("visibility").addClass("lightSpeedIn");
                            setTimeout(function(){
                                $big_award.removeClass("lightSpeedIn");
                            },1000);
                            flag = false;
                        }else{
                            $big_award.addClass("visibility");
                        }
                    }
                });
        },
        ...
        beforeCountdown: function(prizeActList) {
            ...
            me.downloadImg();
            hidenewLoading();
            //me.showBigawards_tip(beginTimeStr);
        },
        // 摇奖结束倒计时
        nowCountdown: function(prizeActList){
            ...
            me.downloadImg();
            hidenewLoading();
            me.showBigawards_tip(prizeActList.pd+" "+prizeActList.st);
        },
        // 跨天摇奖开启倒计时
        ...
        fill: function(data) {
            ...
            if (data.pt == 1) {
                ...
                H.dialog.duiHuanMaLottery.open(data);
            } else if (data.pt == 7) {
        ...
        if(data.result){
            ...
            if(list && list.length > 0){
                ...
                for(var i = 0 ; i < list.length; i++){
                    con += "<li><img src='images/jl-left.png'><span>" + (list[i].ni || "匿名用户") + "中了" + list[i].pn + "</span></li>";
                }
