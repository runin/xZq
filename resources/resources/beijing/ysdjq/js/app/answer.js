(function($) {
	...
	H.utils = {
		...
		resize: function() {
            ...
            var outboxh = width / 3;
			this.$wrapper.css({'height': Math.round(height - 45)});
			this.$outLink.css({'max-height': outHeight+20});
			this.$comments.css({'height': Math.round(height - (outHeight+20) - outboxh)});
	...
	W.callbackArticledetailListHandler = function(data) {
		if (data.code == 0) {
            $(".out-link").append('<img onclick="H.answer.outlink(this)" data-link="' + data.arts[0].gu + '" src="' + (data.arts[0].img).toString() + '" data-collect="true" data-collect-flag="answer-AD" data-collect-desc="弹幕页广告" />');
		}
	};
