            ...
            if ($file_upload.files && $file_upload.files[0]) {
                ...
                reader.onload = function (e) {
                    if (e.target.result) {
                        ...
                        $('.swiper-slide-active').css('height', $('.swiper-slide:first').height());
                        $('.upload-img').attr('src', e.target.result);
                        $(".upload-img").removeAttr("style");
                        $('.upload-img').css({'opacity': '0','padding': '0'});
                ...
                if (evt.target && evt.target.responseText) {
                    ...
                    if(me.uploadProcess) {
                        ...
                        if (uploadImg < minUploadImg) {
                            ...
                            var imgH = minUploadImg;
                            if(uploadImg > imgH){
                                var per = imgH/uploadImg;
                                $('.upload-img').css({'width':windowW * per + 'px','height':imgH + "px"});
                            }
    ...
    H.share = {
        init: function() {
            ...
            if (starNo != 'U') {
                ...
                wxshareData.desc = (familyType + nickname + '寄您了一份祝福，点击查看！');
            }
        ...
        init: function(data) {
            ...
            if (configData[0] == 'U') {
                if (data.ou) {
                    ...
                    Img.src = data.ou;
                    Img.onload = function () {
                        var minshowH = $(window).height()*0.38, showImgH = Img.height;
                        if (showImgH < minshowH) {
                            $('#unpack-dialog .showCard-container img').css('padding-top', ((minshowH - showImgH) / 2));
                        }else{
                            $('#unpack-dialog .showCard-container img').css('padding', '0');
                            var windowW = $(window).width();
                            var imgH = minshowH;
                            var per = imgH/showImgH;
                            if(Img.width > windowW){
                                if(Img.width > Img.height){
                                    $('#unpack-dialog .showCard-container img').css({'width':windowW+ 'px','height':windowW*(Img.height/Img.width) + "px"});
                                }else{
                                    if(minshowH*(Img.width/Img.height) > windowW){
                                        $('#unpack-dialog .showCard-container img').css({'width':windowW+ 'px','height':minshowH + "px"});
                                    }else{
                                        $('#unpack-dialog .showCard-container img').css({'width':minshowH*(Img.width/Img.height)+ 'px','height':minshowH + "px"});
                                    }
                                }
                            }else{
                                $('#unpack-dialog .showCard-container img').css({'width':windowW * per + 'px','height':imgH + "px"});
                            }
                        }
