; (function () {
    window.yao_scope = 'userinfo';
    window.openid = $.fn.cookie(mpappid + '_openid');
    window.headimgurl = $.fn.cookie(mpappid + '_headimgurl');
    window.nickname = $.fn.cookie(mpappid + '_nickname');
    window.expires_in = { expires: 30 };
	
    var getQueryParam = function (name) {
        var currentSearch = decodeURIComponent(window.location.search.slice(1));
        if (currentSearch != '') {
    ...
    $.ajax({
        ...
        success: function (data) {
          ...
          //  window['shaketv'] && shaketv.wxShare(share_img, share_title, share_desc, getUrl());
            ...
            window['shaketv'] && shaketv.wxShare(share_img, share_title, share_desc, getUrl().replace("firstPage", "index"));
//             
//            shaketv.subscribe({
