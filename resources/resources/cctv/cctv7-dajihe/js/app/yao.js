...
(function($) {
	...
	H.yao = {
		...
		luckCallback: function(data){
			...
			if(data.result == false){
				...
				$('#award_tips').removeClass('none').html(textList[Math.floor(i)]);
			...
			if(data.pt == 0){
				...
				var type = parseInt(data.aa, 10);
				if(type == 1){
					// 茶泡泡
					H.chadiandian.show(JSON.stringify(data));
					handled = true;

				}else if(type == 2){
					// 车点点
					H.chediandian.show(JSON.stringify(data));
					handled = true;

				}else if(type == 3){
					// 天天摇
					H.tiantianyao.show(JSON.stringify(data));
					handled = true;
	...
	W.callbackLeftLimitPrizeHandler = function(data){
		if(data.result == true){
			if(data.lc > 0){
				$('#left_tips').removeClass('none');
			}else{
				$('#left_tips').addClass('none');
			}
			$('#left_tips').html('还剩<span style="color:#ff3234;font-weight:bold;font-style: italic;padding: 0 3px;">'+ data.lc +'</span>个奖品，继续加油');
		}
		setTimeout(function(){
			getResult('api/lottery/leftLimitPrize',{
				at : 1
