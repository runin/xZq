        ...
        this.InitEvent = function () {
            ...
            this.subBtn.click(function () {//提交记者稿件
                if (that.checkParam()) { //验证通过
                    ...
                    var url = domain_url + "twoSessions/feeds/reporter/submit?jo=" + that.jo + "&tl=" + that.tl + "&cn=" + that.cn + "&vi=" + that.vi + "&temp=" + new Date().getTime();
                    N.loadData({ url: url,
                        callbackReporterFeedsHandler: function (data) {
                            if (data.result) {
        ...
        this.CheckIsReporter = function () {//检查是否是记者
            ...
            H.dialog.showWin.open("", function (w) {
                ...
                $(".check-btn", w).click(function () {
                    ...
                    N.loadData({ url: domain_url + "twoSessions/feeds/reporter/auth?jo=" + encodeURIComponent(jobN) + "&temp=" + new Date().getTime(), callbackReporterAuthHandler: function (data) {

                        if (data.result) {//验证成功
                            $(".check-tip").text("验证成功！");
                            that.jobN = $(".input-check", w).val().trim(); //保存记者号
                            setTimeout(function () {
                                $(w).remove();
                            }, 500);
                        } else { //验证失败
