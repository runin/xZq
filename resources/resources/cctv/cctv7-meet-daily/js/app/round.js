(function($) {
	H.round = {
		...
		fill_data: function(data) {
			...
			for (var i = 0; i < length; i ++) {
				...
				t._('<li data-au="'+ attrs[i].au + '" class="' + attrs[i].uv + ' type' + attrs[i].at + '" style="background:url(' + attrs[i].ai + ') no-repeat center center;background-size:cover;">')
					...
					._('<a class="lovein_btn" data-collect="true" data-collect-flag="cctv7-meet-love-votebtn" data-collect-desc="投票页-爱心投票按钮"></a>')
				._('</li>');
			};
			$('.main .guest-box ul').html(t.toString());
			$('header').css('height', Math.round(me.windowHeight*0.35));
			$('.main .guest-box ul li').css('height', Math.round(me.windowHeight*0.45));
			if (me.roundNum == 1) {
				$('body').removeClass('round2').addClass('round1');
			} else if(me.roundNum == 2) {
				$('body').removeClass('round1').addClass('round2');
			};
		...
		fill_result: function(data) {
			...
			$('.resulting .guest-box ul li').css('height', Math.round(me.windowHeight*0.45));
			...
			if (me.roundNum == 1) {
				...
				$('body').removeClass('round1').addClass('round2');
				$('.resulting li').each(function(index, el) {
					if ($(this).attr('data-au') == H.round.round2uuid) {
						$(this).find('.lovebox').addClass('lovebox_close');
					};
				});
			};
		...
		event_handler: function() {
			...
			$('.round1 .main .lovebox').click(function(e){
				e.preventDefault();
			...
			$('.round1 .main li').click(function(e){
				e.preventDefault();
			...
			$('.round2 .main .lovebox').click(function(e){
				e.preventDefault();
				H.round.round2uuid = $(this).parent().attr("data-au");
				getResult('meet/vote', {
					openid: openid,
			...
			$('.round2 .main li').click(function(e){
				e.preventDefault();
			...
			$('.main .lovein_btn').click(function(e) {
				e.preventDefault();
				H.round.round2uuid = $(this).parent().attr("data-au");
				getResult('meet/vote', {
					openid: openid,
