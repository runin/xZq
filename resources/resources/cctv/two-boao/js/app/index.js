(function($) {
	...
	H.index = {
		...
		init: function() {
			...
			$('#avatar').find('img').attr('src', headimgurl ? (headimgurl + '/' + yao_avatar_size) : './images/blank.gif');
			$('#myname').text($.fn.cookie(shaketv_appid + '_nickname') || '匿名');
		...
		event: function() {
			...
			$('#btn-send').click(function(e) {
				var $input = $('#input-cmt'),
					...
					nickname = $.fn.cookie(shaketv_appid + '_nickname');
				...
				getResult({
					...
					data: {
						...
						headimgurl: $.fn.cookie(shaketv_appid + '_headimgurl')
					},
					loading: true,
					...
					complete: function(data) {
						...
						var $topic = $('#msg-' + tid),
							...
							comment = '<li class="comment c-box" data-id="'+ data.uid +'"><a>'+ (nickname || '匿名') +'</a>'+ replyTxt +':<span>'+ cmt +'</span></li>';
		...
		addStyle: function() {
			var height = $(window).height(),
				...
				style = '<style>\
							...
							.body-cmt .main { height: '+ (height - 50) +'px; }\
				...
				success: function(data) {
					...
					if (len > 0) {
						var $content = $('#content'),
							...
							$tpl = $(me.topicTpl(items, "homelist"));
		...
		topicTpl: function(items, homelist) {
			var t = simpleTpl(),
				items = items || [];
			...
			for (var i = 0, len = items.length; i < len; i ++) {
				...
				if(homelist) {	
					var ntTime = timestamp(timeline);
					var ct = items[i].ct;
					var ctTime = timestamp(ct);
					if(ntTime>=ctTime && count == true) {
						t._('<div class="history">')
						   ._('<span>以下是2015两会解码</span>')
							 ._('</div>');
					   count = false;
					}
				}
				$(".history").prev().css("border","none");
	...
	H.share = {
		...
		getTitle: function() {
			...
			if (H.index.topicId && nickname) {
				nickname = nickname.split(' ')[0];
				return '我在和'+ nickname +'共议国是，还有'+ (window['pvnum'] || 1) +'人在一起探讨，你不加入？';
			} else {
				return '我已经和'+ (window['pvnum'] || 1) +'人一起加入了央视博鳌解码群策群力讨论，快来看看吧！';
			}
		}
	};
		...
		init: function() {
			...
			this.$scroll.on('scrollEnd',function() {
				if (H.index.canLoadMore && (this.directionY == 1 && this.y < (this.maxScrollY + 30)) && $pullup.hasClass('hidden')) {
					...
					if (H.index.topicId) {
						H.index.fillCmt(H.index.topicId, function() {
