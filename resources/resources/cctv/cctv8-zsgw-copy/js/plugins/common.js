...
var initTag_a = function() {
    $("a").each(function() {
        var that = $(this);
        var href = that.attr("href");
        if (href) {
            if (href.indexOf("javascript:") == -1 && href.indexOf("#myCarousel") == -1) {
                if (href.split("?").length == 2) {
                    href += "&sid=" + storeId;
                    href += "&busId=" + businessId;
                } else {
                    href += "?sid=" + storeId;
                    href += "&busId=" + businessId;
                }
...
$(function() {
    ...
    if (cbUrl.indexOf('cb41faa22e731e9b') < 0 || getQueryString('cb41faa22e731e9b') == "null") {
    ...
    if (typeofAppid == 'undefined' || follow_shaketv_appid == '' || follow_shaketv_appid == null) {
        ...
        if ($("body").attr("data-sr") != "lottery") {
            window['shaketv'] && shaketv.subscribe({
                appid: follow_shaketv_appid2,
                selector: "#div_subscribe_area",
                type: 1
            }, function(returnData) {
                if (returnData.errorCode == 0) {
                    recordUserOperate(openid, "中视购物商城一键关注_成功", "zsgw_subscribe_OK");
                } else if (returnData.errorCode == -1002) {
                    recordUserOperate(openid, "中视购物商城一键关注_用户取消", "zsgw_subscribe_cancel");
                } else if (returnData.errorCode == 9528) {
                    recordUserOperate(openid, "中视购物商城一键关注_用户已经关注", "zsgw_subscribe_already");
                }
            });
        } else {
            window['shaketv'] && shaketv.subscribe({
                ...
                if (returnData.errorCode == 0) {
                    recordUserOperate(openid, "CCTV电视剧一键关注_成功", "cctv_subscribe_OK");
                } else if (returnData.errorCode == -1002) {
                    recordUserOperate(openid, "CCTV电视剧一键关注_用户取消", "cctv_subscribe_cancel");
                } else if (returnData.errorCode == 9528) {
                    recordUserOperate(openid, "CCTV电视剧一键关注_用户已经关注", "cctv_subscribe_already");
                }
            });
        }
    }
    $('body').delegate("*[data-collect='true']", "click", function(e) {
        ...
        if (href && href !== '#') {
            setTimeout(function() {
                if ($(".page-meun").closest("div").hasClass("page-meun") && cbUrl.indexOf('cb41faa22e731e9b') >= 0) {
                    if (href.indexOf("?") >= 0) {
                        window.location.href = href + "&cb41faa22e731e9b=" + localStorage.getItem("cb41faa22e731e9b");
                    } else {
                        window.location.href = href + "?cb41faa22e731e9b=" + localStorage.getItem("cb41faa22e731e9b");
                    }
                } else {
