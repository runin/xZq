...
var showTips = function(word, pos, timer) {
	if (aniTrue) {
		...
		var pos = pos || '2',
		timer = timer || 1500;
		$('body').append('<div class="showtips none"></div>');
		$('.showtips').css({
			'position': 'absolute' ,
			'max-width': '80%' ,
			'top': '60%' ,
		...
		var winW = $(window).width(),
			winH = $(window).height();
		$('.showtips').removeClass('none').css('opacity', '0');
		var showtipsW = $('.showtips').width(),
			showtipsH = $('.showtips').height();
		$('.showtips').css({'margin-left': -showtipsW/2,'top':(winH - showtipsH)/(pos - 0.2)}).removeClass('none');
		$('.showtips').animate({
			'opacity': '1',
			'top': (winH - showtipsH)/pos}, 300, function() {
				setTimeout(function() {
					$('.showtips').animate({'opacity':'0'}, 300, function() {
						$('.showtips').addClass('none').css('top', (winH - showtipsH)/(pos - 0.2));
					});
				}, timer);
				setTimeout(function() {
					$('.showtips').remove();
...
$(function() {
	...
	$('body').delegate("*[data-collect='true']", "click", function(e) {
		...
		// 2015年8月19日10:58:37 《世锦赛》取消点击事件上报点击流
