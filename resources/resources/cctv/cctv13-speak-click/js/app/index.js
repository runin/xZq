(function($) {
    H.index = {
        ...
        event: function() {
            ...
            $('body').delegate('#btn-go', 'click', function(e) {
                ...
                e.preventDefault();
                me.reset();
            }).delegate('#btn-voice', 'click', function(e) {
                e.preventDefault();
                if ($(this).hasClass('recording')) {
                    clearInterval(intervalFlag);
                    $('#btn-voice').removeClass('recording').html('点击此处录制语音');
                    ...
                    $('#btn-voice').addClass('recording').html('点击停止录音');
                    ...
                    intervalFlag = setInterval(function(){
                        ...
                        if (recordTimeLimit == 1) {
                            ...
                            $('.timetips').removeClass('none').find('p').addClass('show').html('还可以说<span>' + (recordTimeLimit-1) + '</span>秒');
                        }
                    }, 1000);
                    return false;
                }
        ...
        fillUserinfo: function() {
            $('.nickname').html(nickname ? nickname : defaultName);
            $('.avatar').attr('src', (headimgurl ? (headimgurl + '/' + yao_avatar_size) : "./images/icon-tvlogo.png"));
            ...
            $.ajax({
                ...
                success : function(data) {
                    if (data.result) {
                        cardUUID = data.cu;
                        $('.info-box').addClass('none');
                        $('.btn-info').addClass('none');
                        $('.btn-again').removeClass('none');
                        if (cardDetailType == 0) {
        ...
        init: function() {
            ...
            swiper = new Swiper('.swiper-container', {
                ...
                onSlideChangeStart: function(swiper){
                    ...
                    me.changeCardInfo(swiper.activeIndex);
                }
            });
            starNo = $('.swiper-slide-active').attr('data-starNO');
        },
            ...
            xhr.addEventListener("load", function(evt) {
                if (evt.target && evt.target.responseText) {
                    ...
                    if(me.uploadProcess) {
                        ...
                        // if (swiperHeight > uploadImg) {
                        //     if (uploadImg > $(window).height()*0.6) {
                        //         $('.upload-img').css('padding-top', '0');
                        //     } else {
                        //         $('.upload-img').css('padding-top', Math.ceil(($(window).height()*0.6 - uploadImg) / 2));
                        //     }
                        //     $('.swiper-wrapper').animate({'height': $(window).height()*0.6}, 500);
                        //     $('.upload-img').parent('.swiper-slide').css({'min-height': $(window).height()*0.6});
                        //     $('.card-wrapper').css({ 'height': Math.ceil($('.card-container').height() + $(window).height()*0.6)});
                        // } else {
                        //     $('.upload-img').css('padding-top', '0');
                        //     $('.swiper-wrapper').animate({'height': $('.upload-img').height()}, 500);
                        //     $('.upload-img').parent('.swiper-slide').css({'min-height': $('.upload-img').height()});
                        //     $('.card-wrapper').css({ 'height': Math.ceil($('.card-container').height() + $('.upload-img').height())});
                        // }
    ...
    H.share = {
        init: function() {
            ...
            wxshareData.title = wxData.title;
        ...
        init: function(data) {
            ...
            if (configData[0] == 'U') {
                if (data.ou) {
                    ...
                    Img.src = data.ou;
                    Img.onload = function () {
                        $('#unpack-dialog .showCard-container img, #unpack-dialog .host-avatar').attr('src', data.ou);
                    ...
                    $('#unpack-dialog .showCard-container img, #unpack-dialog .host-avatar').attr('src', './images/swiper-default.jpg');
                    if (data.hi) {
                        $('#unpack-dialog .host-avatar').attr('src', data.hi + '/' + yao_avatar_size);
                    } else {
                        $('#unpack-dialog .host-avatar').attr('src', './images/icon-tvlogo.png');
                ...
                $('#unpack-dialog .showCard-container img, #unpack-dialog .host-avatar').attr('src', './images/host/host' + parseInt(configData[0]) + '.jpg');
            ...
            $('#unpack-dialog .showCard-container').css('height', $(window).height()*0.62);
            if (data.gt != '') {
                userWordFlag = true;
            ...
            if (data.vi != '') {
                userWordFlag = false;
                if (wxIsReady) {
                    $('#index-dialog').removeClass('none');
                    $('#unpack-dialog').addClass('none');
                } else {
