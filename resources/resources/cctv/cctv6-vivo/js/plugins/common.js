...
var showTips = function(word, pos, timer) {
	if (aniTrue) {
		aniTrue = false;
		var pos = pos || '3', timer = timer || 1200;
		...
		var tipsW = $('.tips').width() + 30, tipsH = $('.tips').height();
		$('.tips').css({'margin-left': -tipsW/2}).removeClass('none');
		$('.tips').animate({'opacity': '1'}, 300, function() {
				setTimeout(function() {
...
function addLotteryCookie() {
    if ($.cookie('canLottery') == null) {
        $.cookie('canLottery', 'true', {
            expires: 1
        })
    }
};

function delLotteryCookie() {
    if ($.cookie('canLottery') != null) {
        $.cookie('canLottery', '', {
            expires: -1
        })
...
function addGameErrorCookie(number, url, targetUrl) {
    if (url != undefined && targetUrl != undefined) {} else {
        alert('url和targetUrl不能为空值');
        return
    } if (number == undefined) {
        number = 3
    } else if (number <= 2) {
        alert('请不要传递小于2的number参数');
        return
    }
    if ($.cookie('gameError') == null) {
        $.cookie('gameError', 1, {
            expires: 1
        });
        setTimeout(function() {
        ...
        var oldNum = parseInt($.cookie('gameError'));
        if (oldNum >= number - 1) {
            delGameErrorCookie();
            addLotteryCookie();
            $('canvas').attr({
...
var getUrl = function(openid) {
	...
	href = delQueStr(href, "cg");
	return add_yao_prefix(href);
};
var getnewUrl = function(openid) {
	var href = window.location.href;
	// href = add_param(href,"auid", auid, true);
	href = add_param(href.replace(/[^\/]*\.html/i, 'headline_detail.html'), 'resopenid', hex_md5(openid), true);
	href = add_param(href, 'from', 'share', true);
	href = delQueStr(href, "openid");
	href = delQueStr(href, "headimgurl");
	href = delQueStr(href, "nickname");
	...
	$.ajax({
		...
		success : function(data) {
			...
			if ($('body').hasClass('H-detail')) {
				window['shaketv'] && shaketv.wxShare(share_img, share_title, share_desc, getnewUrl(openid));
			} else {
