	...
	H.answer = {
				...
				t._('<li data-qriu="'+ qitems[i].qriu +'" data-qcode="'+ qcode +'" data-ruid="'+ ruid +'" data-stime="'+ timestamp(qitems[i].qst) +'" data-etime="'+ timestamp(qitems[i].qet) +'" id="question-'+ qitems[i].quid +'" data-quid="'+ qitems[i].quid +'" >')
					...
					._('<div class="dorA">')
						...
						for (var j = 0, jlen = aitems.length; j < jlen; j ++) {
							t._('<a class="q-item" href="#" id="answer-'+ aitems[j].auid +'" data-auid="'+ aitems[j].auid +'" data-collect="true" data-collect-flag="tv-jiangsu-week-luck-answer-item" data-collect-desc="答题页-答案按钮"><i></i>'+ aitems[j].at +'</a>')
						}
					t._('</div>')
		...
		itemBtnclick: function() {
			...
			$('.q-item').click(function(e) {
				...
				var qriu = $(this).parent('.dorA').parent('li').attr('data-qriu');
				if (qriu != me.collectAnswer) {
					showTips('回答错了，再试试吧！');
					return;
				}
