...
var showTips = function(tips, timer) {
	...
	if (aniTrue) {
		aniTrue = false;
		$('body').append('<div class="tips" style="position:fixed;max-width:80%;z-index:999999999999;color:#FFF;padding:10px 15px;font-size:16px;border-radius:3px;background:rgba(0,0,0,.7);text-align:center;opacity:0">' + tips + '</div>');
		var winW = $(window).width(), winH = $(window).height(), tipsW = $('.tips').width(), tipsH = $('.tips').height(), timer = timer || 1200;
		$('.tips').css({'left':(winW - tipsW)/2, 'top':(winH - tipsH)/2}).animate({'opacity': '1'}, 300, function() {
			setTimeout(function() {
				$('.tips').animate({'opacity':'0'}, 100, function() {
					$('.tips').remove();
					aniTrue = true;
				});
			}, timer);
