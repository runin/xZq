...
$(function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if ($("body").attr("data-sr") == "share") {
				new Authorize({callBackPage:"share.html"}).init(check_weixin_login());
			}else{
		...
		if ($("body").attr("data-sr") == "vote") {
			window['shaketv'] && shaketv.subscribe({
				...
				if (returnData.errorCode == 0) {
					recordUserOperate(openid, "惊喜连连一键关注_成功", "jxll_subscribe_OK");
				} else if (returnData.errorCode == -1002) {
					recordUserOperate(openid, "惊喜连连一键关注_用户取消", "jxll_subscribe_cancel");
				} else if (returnData.errorCode == 9528) {
					recordUserOperate(openid, "惊喜连连一键关注_用户已经关注", "jxll_subscribe_already");
				}
			});
		} else {
			window['shaketv'] && shaketv.subscribe({
				...
				if (returnData.errorCode == 0) {
					recordUserOperate(openid, "央视财经一键关注_成功", "cctv2_subscribe_OK");
				} else if (returnData.errorCode == -1002) {
					recordUserOperate(openid, "央视财经一键关注_用户取消", "cctv2_subscribe_cancel");
				} else if (returnData.errorCode == 9528) {
					recordUserOperate(openid, "央视财经一键关注_用户已经关注", "cctv2_subscribe_already");
				}
			});
		}
	}
