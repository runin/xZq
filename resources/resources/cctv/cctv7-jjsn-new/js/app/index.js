(function($) {
	H.talk = {
        ...
        init_size : function(){
        	var bodyH =$(window).height()-$(".preme").height(),hex_professOpenId =[];
...
//           $(".time-text").removeClass("hidden");
    ...
    W.callbackCommentsTopicInfo = function(data){
        if(data.code == 0){
            ...
            setInterval(function(){
                H.talk.account_num();
            },Math.ceil(3000*Math.random() + 5000));
            $(".topic").html(item.t);
            H.talk.init_size();
            H.talk.current_time();
            ...
            if(items.length > 0){
                ...
                for(var i = items.length-1;i >= 0;i--){
                    ...
                    t._('<li class="'+ state +'">')
                    	._('<p class="tar">'+n+'</p>')
                        ._('<div class="ron">')
                         	._('<div class="head"><img src="'+h+'" onerror=$(this).attr("src","images/head.jpg") ></div>')
