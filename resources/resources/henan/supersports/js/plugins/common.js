...
var showLoading = shownewLoading = function($container, tips) {
    var t = simpleTpl(),
        ...
        $loading = $container ? $container.find('#qy_wrapper') : $('body').children('#qy_wrapper'),
        tips = tips || '努力加载中...';
    if ($loading.length > 0) $loading.remove();
    t._('<section id="qy_wrapper"><section id="qy_loading" class="qy-loading">')
...
var hideLoading = hidenewLoading = function($container) {
    if ($container) {
        $container.find('#qy_wrapper').remove();
    } else {
        $('body').children('#qy_wrapper').remove();
    }
};
...
var getUrl = function(openid) {
    ...
    href = add_param(href.replace(/[^\/]*\.html/i, 'index.html'), 'resopenid', hex_md5(openid), true);
    ...
    href = delQueStr(href, "pid");

    // return add_yao_prefix(href);
    return href;
};

var updateUrl = function(sharePage, cu) {
    var href = window.location.href;
        ...
        success : function(data) {
            if (!data.result) {
                if (location.href.indexOf(sharePage) < 0) {
                    ...
                    var href = data.redirect.replace('index.html', sharePage);
                    location.href = href;
                }
                return;
            }
            ...
            if (location.href.indexOf(sharePage) < 0) {
                ...
                new Authorize({callBackPage:sharePage}).init();
            }
        },
        error : function() {
            if (location.href.indexOf(sharePage) < 0) {
