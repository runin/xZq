...
(function($) {
    H.lottery = {
        ...
        event: function() {
			...
			$("#btn-return").click(function(e){
				e.preventDefault();
				toUrl('index.html?markJump=yaoClick');
			})
        },
        shake: function() {
    ...
    W.commonApiSDPVHander = function(data){
	 ...
	 W.callbackLinesDiyInfoHandler = function(data){
		if(data.code == 0&&data.gitems[2]){
			$(".logo").find("img").attr("src",data.gitems[2].is);
			if(data.gitems[2].info){
				$(".logo").attr("data-href",data.gitems[2].info).removeClass("none");
			}
		}else{
			$(".logo").addClass("none");
		}
