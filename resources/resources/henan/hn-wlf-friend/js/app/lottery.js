(function($) {
    ...
    W.commonApiPromotionHandler = function(data){
        if (data.code == 0 && data.desc && data.url) {
            ...
            var index = localStorage.getItem("wlf_link_index")?(localStorage.getItem("wlf_link_index")*1):0;
            if(index >= url_list.length){
                index = 0;
            }
            $('#ddtj').attr('data-collect-flag', "lottery-link-btn-"+index).attr("data-collect-desc","摇奖页-吸粉链接-"+index);
            $('#ddtj').attr('href', "javascript:void(0);").removeClass('none');
            $('.ddtj').text(desc_list[index] || '');
            $('#ddtj').click(function(){
                location.href = url_list[index];
                localStorage.setItem("wlf_link_index", ++index);
            });
    ...
    W.callbackLinesDiyInfoHandler = function(data){
        ...
        //ib:大图
        if(data.code == 0){
            $("#gg").attr("src",data.gitems[2].ib).removeClass("none");
            if(data.gitems[2].mu){
                $("#gg").click(function(e){
                    e.preventDefault();
                    toUrl(data.gitems[2].mu);
                });
            }
        }
    }
