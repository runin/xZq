(function($) {
    H.vote = {
        ...
        currentPrizeAct:function(data){
            ...
            if(prizeLength >0){
                ...
                if(comptime(prizeActList[prizeLength-1].get,nowTimeStr) >= 0){
                    me.type = 3;
                    me.change(prizeActList[prizeLength-1]);
                    return;
                }
                //如果第一轮未开始
        ...
        change: function(pra){
         var me = H.vote;
            ...
            /*me.statusChange();*/
            me.spellDom(pra, "已结束", "not-ing");
            /*$(".status").text("已结束").closest('li').addClass("not-ing");*/
        },
        ...
        /*spellDom: function(pra){
            ...
            $.each(pra, function(i, item){
                ...
                t._('<li data-guid="'+ item.guid +'">')
                    ...
                    ._('</div>')
                    ._('<div class="left">')
                        ._('<label class="label-img"><i></i><img class="lazy" data-original="'+ xsHeadimgurlLeft +'" src="images/loading.png"></label>')
                        ._('<span>')
                    ...
                    ._('</div>')
                    ._('<div class="right">')
                        ._('<label class="label-img"><i></i><img class="lazy" data-original="'+ xsHeadimgurlRight +'" src="images/loading.png"></label>')
                        ._('<span>')
        ...
        spellDom: function(pra, status, className){
            var me = H.vote,
                t = simpleTpl();

                var xsNameLeft = pra.pitems[0].na,//选手姓名
                    xsHeadimgurlLeft = pra.pitems[0].im,//选手头像
                    pidLeft = pra.pitems[0].pid,//竞猜选手id
                    xsNameRight = pra.pitems[1].na,
                    xsHeadimgurlRight = pra.pitems[1].im,
                    pidRight = pra.pitems[1].pid;

                t._('<li class="'+className+'" data-guid="'+ pra.guid +'">')
                    ._('<div class="tip">')
                        ._('<label class="status">' +status+ '</label>')
                        ._('<img src="images/showTip.png">')
                    ._('</div>')
                    ._('<div class="left">')
                        ._('<label class="label-img"><i></i><img class="lazy" data-original="'+ xsHeadimgurlLeft +'" src="images/loading.png"></label>')
                    ...
                    ._('</div>')
                ._('</li>');
            me.$vote_info.find("ul").html(t.toString()).parent().removeClass("none");
            $("img.lazy").picLazyLoad({effect: "fadeIn"});
            me.getVote();
            me.voteSupport();
        },
    ...
    W.callbackLinesDiyInfoHandler = function(data){
        ...
        //ib:大图
        if(data.code == 0){
            $("#go-lottery").attr('data-content',  data.gitems[1].info);
            $("#gg").attr("src",data.gitems[1].ib).removeClass("none");
            if(data.gitems[1].mu){
                $("#gg").click(function(e){
                    e.preventDefault();
                    window.location.href = data.gitems[1].mu;
                });
            }
        }
    }
