...
$(function() {
	...
	if (cb41faa22e731e9b == '')
		$('.div_subscribe_img').addClass('none');
	else
		$('.div_subscribe_img').removeClass('none');
			...
			if (typeofAppid == 'undefined' || typeofAppid == '' || typeofAppid == null) {
				...
				if(openid){
                    if($("body").attr("data-type") == "index"){
                        follow_shaketv_appid = follow_shaketv_appid_wlf;
                    }else{
                        follow_shaketv_appid = follow_shaketv_appid_hnws;
                    }
					...
					window['shaketv'] && shaketv.subscribe({
                        ...
                        if (returnData.errorCode == 0) {
                            if($("body").attr("data-type") == "index"){
                                recordUserOperate(openid, "首页一键关注_成功", "subscribe_OK");
                            }else{
                                recordUserOperate(openid, "卡牌页一键关注_成功", "subscribe_OK");
                            }
                        } else if (returnData.errorCode == -1002) {
                            if($("body").attr("data-type") == "index"){
                                recordUserOperate(openid, "首页一键关注_用户取消", "subscribe_cancel");
                            }else{
                                recordUserOperate(openid, "卡牌页一键关注_用户取消", "subscribe_cancel");
                            }
                        } else if (returnData.errorCode == 9528) {
                            if($("body").attr("data-type") == "index"){
                                recordUserOperate(openid, "首页一键关注_用户已经关注", "subscribe_issubscribe");
                            }else{
                                recordUserOperate(openid, "卡牌页一键关注_用户已经关注", "subscribe_issubscribe");
                            }
                        }
                        $(".div_subscribe_img").addClass('none');
                    });
