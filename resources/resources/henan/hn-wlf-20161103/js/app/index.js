(function($){
    H.index = {
                        ...
                        for(var i = 0;(i<=data.items.length - 1);i++){
                            ...
                            if((serverT>st)&&(serverT<et)){
                                ...
                                for(var a = 0; a < data.items[i].pitems.length;a++){
                                    ...
                                    infoData += '<div class="swiper-slide"><img src="' + data.items[i].pitems[a].im + '" onclick="H.index.outlink(this)" data-link="' + data.items[i].pitems[a].im2 + '" data-collect="true" data-collect-flag="outer-btn' + (data.items[i].pitems[a].na?data.items[i].pitems[a].na:a) + '" data-collect-desc="' + (data.items[i].pitems[a].ni) + '_外跳按钮" >';
        ...
        change: function() {
            ...
            $(".countdown").css({"-webkit-animation":""});
            $(".tolottery").addClass("none");
            hidenewLoading();
    ...
    W.callbackLinesDiyInfoHandler = function(data){
    	if(data.code == 0 && data.gitems[0].ib){
            $('#ddtj').attr('data-collect-flag', "wlf-vote-link-btn").attr("data-collect-desc","投票页-吸粉链接");
            $('#ddtj').click(function(){
                location.href = data.gitems[0].ib;
            });
