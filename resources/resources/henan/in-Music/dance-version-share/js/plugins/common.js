...
var showTips = function(tips, timer) {
    ...
    if (showTipsFlag) {
        showTipsFlag = false;
        $('body').append('<div class="showTips" style="position:fixed;max-width:80%;z-index:999999999999;color:rgb(39, 38, 38);padding:12px 18px;font-size:15px;border-radius:3px;background:rgba(255,255,255,.7);text-align:center;opacity:0">' + tips + '</div>');
        ...
        var runTime = function() {
            ...
            vthis.each(function() {
                ...
                var showTime = function(rT, showTpl) {
                    ...
                    if ($(".fetal-D label").text() == 00) {
                        $('.countdown').addClass('day');
                    }else{
                        $('.countdown').removeClass('day');
                    }

                    if ($(".fetal-H label").text() == 00) {
...
$(function() {
    ...
    window.getNewUrl = function(openid, sign) {
        var href = window.location.href;

        href = add_param(href,"sign", sign, true);
        href = href.replace(/[^\/]*\.html/i, 'details.html');
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            if($("body").attr("data-type") == "rank"){

                var name = W.localStorage.getItem("nameStar-"+ openid),
                    base = W.localStorage.getItem("guid-"+ H.rank.guid +"pid-"+ H.rank.pid)*1 || 0;
                if (base && base > 0) {
                    share_title = '【我为' + (name || '') + '摇出' + (base || '') + '个赞】看看你能摇多少！';
                    share_desc = '李宇春、林志颖、方大同、陈翔、徐浩等你为TA摇';
                    // 一键分享
