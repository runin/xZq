				...
				if(openid){
					...
					window['shaketv'] && shaketv.subscribe({
                        ...
                        if (returnData.errorCode == 0) {
                            if($("body").attr("data-type") == "wlf"){
                                recordUserOperate(openid, "河南卫视一键关注_成功", "subscribe_OK");
                            }else{
                                recordUserOperate(openid, "勇士的荣耀一键关注_成功", "subscribe_OK");
                            }
                        } else if (returnData.errorCode == -1002) {
                            if($("body").attr("data-type") == "wlf"){
                                recordUserOperate(openid, "河南卫视一键关注_用户取消", "subscribe_OK");
                            }else{
                                recordUserOperate(openid, "勇士的荣耀一键关注_用户取消", "subscribe_OK");
                            }
                        } else if (returnData.errorCode == 9528) {
                            if($("body").attr("data-type") == "wlf"){
                                recordUserOperate(openid, "河南卫视一键关注_用户已经关注", "subscribe_OK");
                            }else{
                                recordUserOperate(openid, "勇士的荣耀一键关注_用户已经关注", "subscribe_OK");
                            }
                        }
					});
				}
