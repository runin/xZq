(function($) {
        ...
        if(data.result){
            ...
            if(list && list.length > 0){
                ...
                for(var i = 0 ; i < list.length; i++){
                    con += "<li>" + (list[i].ni || "匿名用户") + "   中了  <span style='color:#e6c965'>" + list[i].pn + "</span></li>";
                }
    ...
    W.commonApiPromotionHandler = function(data){
        if (data.code == 0 && data.desc && data.url) {
            ...
            $('.ddtj').text(data.desc.substring(0,de) || '');
            $("#ddtj").click(function(){
                showLoading();
    ...
    W.callbackArticledetailListHandler = function(data) {
        if(data == undefined){
            ...
            if(data.code == 0){
                if($("body").hasClass('body-vote')){
                    $("header").children().first().before('<img class="ad" src="' + (data.arts[1].img).toString() + '" />');
                }else{
                    $(".footer").children().first().before('<img class="ad" src="' + (data.arts[0].img).toString() + '" />');
                ...
                if($("body").hasClass('body-vote')){
                    //$(".go-talk").css("top","20px");
                    //$(".go-lottery").css("top","20px");
