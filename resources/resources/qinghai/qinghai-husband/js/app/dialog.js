(function($) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.code == 0){
                            ...
                            $(".linkout").attr("href",jumpUrl).removeClass("none");
                        }else{
        ...
        guide: {
            ...
            tpl: function() {
                ...
                t._('<section class="modal modal-guide" id="guide-dialog">')
                    ...
                    ._('<div class="guide-content"><p><i>1</i>锁定青海卫视《大丈夫》</p>')
        ...
        lottery: {
            ...
            open: function(data) {
                ...
                if(data.pt == 7){
                    ...
                    document.getElementById('aw').onload = function () {
                        inheight = ($(".contact").height() + $(".award-img").height() + 35) + $(".awardwin-tips").height() + (winH * 0.12);
                        if(inheight >= winH){
                ...
                if (data != null && data != '') {
                    if (data.result) {
                        if (data.pt === 1) { //实物奖品
                            ...
                            this.$dialog.find('.award-win').removeClass('none');
                            this.$dialog.find('.contact').removeClass('redbag');
                        } else if (data.pt === 6) { //流量红包
                            this.$dialog.find(".lottery-dialog").addClass("award-bg");
                            this.$dialog.find('.awardwin-tips').html(data.pn || '');
                            this.$dialog.find('.award-tip').html('请正确填写手机号码，三个工作日内将会有工作人员与您联系奖品发放事宜');
                            this.$dialog.find('img').attr('src', (data.pi || ''));
                            this.$dialog.find('.q-name').addClass("none");
                            this.$dialog.find('.mobile').val(data.ph || '');
                            this.$dialog.find('.q-address').addClass("none");
            ...
            check: function() {
                var $mobile = $('.mobile'),
                    ...
                    $name = $('.name'),
                    name = $.trim($name.val());
                if(H.dialog.lottery.pt == 5 || H.dialog.lottery.pt == 6){
                    if (!/^\d{11}$/.test(mobile)) {
                        showTips('这手机号，可打不通...');
                        return false;
                    }
    ...
    W.callbackLotteryLuckHandler = function(data) {
        if(data.result && H.talk !== undefined){
            H.dialog.lottery.open(data);
            H.dialog.PV = data.pv;
        }else if(data.code == 1 && H.talk !== undefined){
