$(function() {
	...
	window.callbackClueHandler = function(data) {
		...
		if (data.code == 0) {
			...
			window.location.href = 'lottery.html?uid='+actid+'&cid='+data.cuuid;
		} else {
			alert(data.message);
		}
	}
...
function fileSelected() {
    ...
    if ($file_upload.files && $file_upload.files[0]) {
        ...
        t._('<a href="#" id="'+ img_id +'" class="img-preview loading" data-collect="true" data-collect-flag="js-eyes-baoliao-preview-btn" data-collect-desc="爆料页面 预览图片按钮">')
			._('<span class="lc"><i class="uploading"></i></span>')
			._('<i class="close"  data-collect="true" data-collect-flag="js-eyes-baoliao-remove-img-btn" data-collect-desc="爆料页面 删除上传的图片按钮" data-stoppropagation="true"></i>')
