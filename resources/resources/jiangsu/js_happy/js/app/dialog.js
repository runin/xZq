	...
	H.dialog = {
		...
		lottery: {
			...
			event: function() {
				...
				$('#btn-award').click(function(e) {
					...
					//实物奖品
						...
						showTips("领取成功");
					//外链奖品
					}else if(H.dialog.lottery.pt == 3){//实物2
					   	if(!H.dialog.lottery.checkid()) return;
					    getResult('api/lottery/award', {
							nn: nickname ? encodeURIComponent(nickname) : "",
							hi: headimgurl ? headimgurl : "",
							oi: openid,
						...
						$('.lottery-dialog').animate({
							'opacity':'0'
						}, 500, function(){
							$('.lottery-dialog').addClass('none');
						});
						$('.extra').css({'opacity':'0'}).removeClass('none').animate({'opacity':'1'}, 500);
				...
				this.$dialog.find('.btn-close,#btn-back,.btn-qrclose').click(function(e) {
					e.preventDefault();
				...
				this.$dialog.find('.btn-qrclose').click(function(e) {
					e.preventDefault();
					var me = H.dialog.lottery;
					me.$dialog.find('.extra').addClass('disphide');
					setTimeout(function(){
						H.dialog.lottery.reset();
					},500);
				});
			...
			update: function(data) {
				...
				if (data.pt === 1) { //实物奖品
				...
				} else if (data.pt === 3) { //实物2
					me.$dialog.find('.lottery-good').removeClass("lottery-card");
					me.$dialog.find('.extra-name').val(data.rn || '');
					me.$dialog.find('.extra-tel').val(data.ph || '');
 					me.$dialog.find('.extra-id').val('');
 					me.$dialog.find('.extra-tips1').html(data.aw || '');
 					me.$dialog.find('.extra-tips2').html(data.al || '');
					me.$dialog.find('.content2').removeClass("none");
					me.$dialog.find('.qrcode-tt').html(data.aw || '');
					if (data.ru && data.ruid) {
						$('#lottery-dialog').removeClass('qrerror');
						var canvas = qrSeed.canvas({
							data: data.ru + '?koi=' + openid + '&kru=' + data.ruid,
							size: 230,
							foreground: [
						        {style: '#222'},
						        {row: 0, rows: 7, col: 0, cols: 7, style: '#333'},
						        {row: -7, rows: 7, col: 0, cols: 7, style: '#333'},
						        {row: 0, rows: 7, col: -7, cols: 7, style: '#333'},
						        {row: 2, rows: 3, col: 2, cols: 3, style: '#444'},
						        {row: -5, rows: 3, col: 2, cols: 3, style: '#444'},
						        {row: 2, rows: 3, col: -5, cols: 3, style: '#444'},
						      ],
						      background: '#FFFFFF',
						});
						$('.qrbox').append(canvas);
					} else {
						$('#lottery-dialog').addClass('qrerror');
					}
			    	...
			    	$('#btn-award img').attr('src', 'images/btn-zdl.png');
			    }
			},
			check: function() { 
				var me = this;
			...
			checkid: function() {
				var me = this;
			    var $tel = $('.extra-tel'),$id = $('.extra-id'),$name = $('.extra-name');
					me.name = $.trim($name.val());
					me.mobile = $.trim($tel.val());
 					me.id = $.trim($id.val());
				if (me.name.length > 20 || me.name.length == 0) {
					showTips('请输入您的姓名，不要超过20字哦!');
					$("#btn-award").removeClass("requesting");
				...
				if (me.id.length > 0) {
					if (!/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(me.id)) {
						showTips('请正确填写身份证号');
						$("#btn-award").removeClass("requesting");
						return false;
					}
			...
			tpl: function() {
				...
				t._('<section class="modal" id="lottery-dialog">')
					._('<div class="dialog lottery-dialog">')
						...
						._('<div class="dialog-assist">')
							...
							._('<div class="award-img">')
								...
								._('</div>')
								._('<div class="content2 none">')
									._('<p class="extra-tips1"></p>')
									._('<p class="extra-tips2"></p>')
									._('<h5>请填写您的联系方式以便顺利领奖</h5>')
									._('<input type="text" class="extra-name" placeholder="姓名" />')
									._('<input type="tel" class="extra-tel" placeholder="电话" />')
									._('<input type="text" class="extra-id" placeholder="身份证号（非必填）" />')
								._('</div>')
							._('</div>')
						._('</div>')
					...
					._('</div>')
					._('<div class="extra none">')
						._('<a href="#" class="btn-qrclose"><img src="./images/btn-qrclose.png"></a>')
						._('<h6><img src="./images/tips-qrcode.png"></h6>')
						._('<p class="qrcode-tt"></p>')
						._('<div class="qrbox"></div>')
					._('</div>')
				._('</section>');
				return t.toString();
			}
