...
$(function() {
    ...
    $.ajax({
        ...
        success : function(data) {
            if (!data.result){
                ...
                if (location.href.indexOf('collect.html') >= 0) {
                    href = add_param(href, 'koi', getQueryString('koi'), true);
                    href = add_param(href, 'kru', getQueryString('kru'), true);
                    location.href = href.replace('index.html', 'collect.html');
                } else if (location.href.indexOf('check.html') >= 0) {
                    href = add_param(href, 'koi', getQueryString('koi'), true);
                    href = add_param(href, 'kru', getQueryString('kru'), true);
                    location.href = href.replace('index.html', 'check.html');
                } else {
                    location.href = data.redirect;
                }
                ...
                //         return;
                //     } else {
                //         var href = data.redirect.replace(/[^\/]*\.html/i, sharePage),
                //             cardRU = getQueryString('cardRU') || '',
                //             cardPU = getQueryString('cardPU') || '',
                //             userName = getQueryString('userName') || encodeURIComponent('幸福剧场观众'),
                //             resopenid = getQueryString('resopenid') || '';
                //         href = add_param(href, 'cardRU', cardRU, true);
                //         href = add_param(href, 'cardPU', cardPU, true);
                //         href = add_param(href, 'userName', userName, true);
                //         href = add_param(href, 'resopenid', resopenid, true);
                //         location.href = href;
                //         return;
                //     }
                // }
            ...
            if (location.href.indexOf('collect.html') >= 0) {
                new Authorize({callBackPage:'collect.html'}).init(check_weixin_login());
            } else if (location.href.indexOf('check.html') >= 0) {
                new Authorize({callBackPage:'check.html'}).init(check_weixin_login());
            } else {
    ...
    $('#btn-answer').click(function(e) {
        ...
        $('#nav').removeClass('nav-shop nav-card').addClass('nav-answer');
        toUrl('answer.html');
    });
    $('#btn-card').click(function(e) {
        e.preventDefault();
        if ($(this).hasClass('active')) return;
        $('#nav').removeClass('nav-shop nav-answer').addClass('nav-card');
        toUrl('card.html');
    });
    $('#btn-shop').click(function(e) {
        e.preventDefault();
        if ($(this).hasClass('active')) return;
        $('#nav').removeClass('nav-answer nav-card').addClass('nav-shop');
        toUrl('shop.html');
    });
});