...
(function(w, $) {
    ...
    $.fn.countDown = function(obj) {
        var defaultParam = {
            ...
            timeInterval: 200, //倒数时间间隔 默认1000 毫秒
            countDownFn: null, //到计时函数，可能触发多次
            atTimeFn: null, //时间段在当前时间内时触发的函数，每隔timeInterval都会会触发多少次
            inQuantumFn: null, //时间段在当前时间内时触发的函数，时间段内只执行一次 多少次时间断执行多少次
            endFn: null, //整个倒计时结束触发的函数
            useSystemTime: true //默认用系统时间校正
        };
        ...
        var o = {
            init: function(systemTime, timeDiff) { //主函数
                        ...
                        for (var i = countIndex, len = p.timeArr.length; i < len; i++) {
                            ...
                            if (t < st) {
                                (function() {
                                    ...
                                    $(that).html(tmp);
                                    p.countDownFn && p.countDownFn(that, tmp, p.timeArr[i].index,st,et,t,o);
                                }());
                                break;
                            }
                            if (st < t && t < et) {
                                ...
                                var tmp = o.showTime(et - t);
                                $(that).html(tmp);
                                if (!p.atTimeFn["OnTimeRun" + p.timeArr[i].index]) {
                                    p.atTimeFn["OnTimeRun" + p.timeArr[i].index] = true;
                                    p.inQuantumFn && p.inQuantumFn(that, p.timeArr[i].index);
                                }
                                p.atTimeFn && p.atTimeFn(that, p.timeArr[i].index, startTime, o, t, et);
                            ...
                            if (t > et) {
                                ...
                                (function() {
                                    if (countIndex == p.timeArr.length) {
                                        ...
                                        if (p.endFn) {
                                            p.endFn(that, p.timeArr[i].index, o, t);
                                        }
                                    }
                                }());
                                break;
                ...
                window.intervalt = setInterval(downFn, p.timeInterval);
            },
            sort: function() { //排序函数
        ...
        if (o.useSystemTime) {
            o.loadData({
                url: p.getSystemTimeUrl,
                callbackTimeStrHandler: function(data) {
            ...
            systemTime = new Date().getTime(); //赋值系统时间
            if (!/^\d+$/.test(systemTime)) { //支持字符串类型
                systemTime = o.timestamp(systemTime);
            }
            timeDiff = new Date().getTime() - systemTime;
