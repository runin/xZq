...
//  'use strict';
    ...
    function init() {
        ...
        if (options.arrow) {
            $pageArr.append('<span class="parallax-arrow left"></span>');
            $($pageArr[0]).find('.parallax-arrow.left').remove();

            $pageArr.append('<span class="parallax-arrow right"></span>');
            $($pageArr[pageCount-1]).find('.parallax-arrow.right').remove();
        }
    }
    ...
    function onMove(e) {
        ...
        if (options.drag && !isHeadOrTail()) { // 拖拽时调用
            dragToMove();
        } else if (isHeadOrTail()) {
        	$pages.css("-webkit-transform", "matrix(1, 0, 0, 1, " + offset + ", 0)");
        }
        stage = 2;
    }
    ...
    function isHeadOrTail() {
    	var last_index = $('.page').index($('.page').not(function() {
			return $(this).hasClass('none');
		}).last());
        if (options.direction === 'horizontal') {
            if ((endPos >= startPos && curPage === 0) ||
                (endPos <= startPos && curPage === last_index)) {
                return true;
            }
    ...
    $(document)
        ...
        .on('webkitAnimationEnd webkitTransitionEnd', '.pages', function() {
			...
			if (direction !== 'stay') {
				...
				$pageArr.length > 0 && $($pageArr.removeClass('current').get(curPage)).addClass('current');
