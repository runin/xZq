...
Lottery.prototype = {
	...
	getTransparentPercent: function(l, c, k) {
		var b = l.getImageData(0, 0, c, k),
			d = b.data,
			e = [];
		for (var g = 0, f = d.length; g < f; g += 4) {
			var h = d[g + 3];
			if (h < 128) {
				e.push(g)
			}
		}
		return (e.length / (d.length / 4) * 100).toFixed(2)
	},
	resizeCanvas: function(b, c, a) {
		b.width = c;
		b.height = a;
		b.getContext("2d").clearRect(0, 0, c, a)
	},
	resizeCanvas_w: function(b, c, a) {
		b.width = c;
		b.height = a;
		b.getContext("2d").clearRect(0, 0, c, a);
		if (this.vail) {
	...
	drawPoint: function(a, d, b) {
		var c = $(this.mask).offset();
		a = a - c.left;
		d = d - c.top;
		this.maskCtx.beginPath();
		this.maskCtx.arc(a, d, 30, 0, Math.PI * 2);
		this.maskCtx.fill();
		this.maskCtx.beginPath();
		...
		this.maskCtx.lineTo(a, d);
		this.maskCtx.stroke();
		this.lastPosition = [a, d];
	...
	bindEvent: function() {
		...
		var b = c ? "touchstart" : "mousedown";
		var d = c ? "touchmove" : "mousemove";
		if (!c) {
			var a = false;
			e.conNode.addEventListener("mouseup", function(g) {
				g.preventDefault();
				a = false;
			...
			e.conNode.addEventListener("touchmove", function(f) {
				if (a) {
					f.preventDefault()
				}
				if (f.cancelable) {
			...
			e.conNode.addEventListener("touchend", function(g) {
				a = false;
				var f = e.getTransparentPercent(e.maskCtx, e.width, e.height);
				if (f >= e.percent) {
					if (typeof(e.drawPercentCallback) == "function") {
		...
		this.mask.addEventListener(b, function(g) {
			g.preventDefault();
			a = true;
			var f = (c ? g.touches[0].pageX : g.pageX || g.x);
			var h = (c ? g.touches[0].pageY : g.pageY || g.y);
			e.drawPoint(f, h, a)
		}, false);
		this.mask.addEventListener(d, function(g) {
			g.preventDefault();
			if (!a) {
				return false
			}
	...
	drawLottery: function() {
		if (this.lotteryType == "image") {
			var b = new Image(),
				c = this;
			b.onload = function() {
				var e = c.resize(this.width, this.height, c.width, c.height),
					d = Math.ceil((c.width - e.width) / 2),
					f = Math.ceil((c.height - e.height) / 2);
				this.width = c.width;
				this.height = c.height;
				c.resizeCanvas(c.background, c.width, c.height);
				c.backCtx.drawImage(this, d, f, e.width, e.height);
				c.drawMask()
			};
			b.src = this.lottery
		} else {
	...
	init: function(a, b) {
		...
		if (this.vail) {
			this.background = this.background || this.createElement("canvas", {
				style: "position:absolute;left:0;top:0px;background-color:transparent;display:none",
				id: "lottery-mask"
			})
		}
