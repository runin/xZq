		   ...
		   wx_card:function(){
			...
			update: function(data) {
				...
				if (data.pt === 6) {
			    	...
			    	// code
 					me.$dialog.find('.t-codes').text(data.cc || '');
					me.$dialog.find('.contact-code').removeClass("none");
			    }else if (data.pt === 9){
			    	H.dialog.lottery.ru = data.ru;
			    }else if (data.pt === 18){
			    }
...
//							 ._('<a href="#" class="btn-close" data-collect="true" data-collect-flag="lottery-dialog-closebtn" data-collect-desc="领奖弹层-关闭按钮"></a>'))
							._('<div class="award-img">')
								._('<img id="aw" src="" />')
								._('<div class="contact-code none">')
									._('<h4 class="award-tip">请截屏此页，需凭此码兑换奖品</h4>')
									._('<p class="code-box">兑换码：<label class="t-codes"></label></p>')
								._('</div>')
