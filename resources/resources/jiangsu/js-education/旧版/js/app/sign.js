...
+(function($) {
	...
	H.sign = {
		...
		signqdbtnFn: function() {
			var that = this;
			signClss.$signQdBtn.click(function() {
				if($("body").find(".pop-sign").length > 0) {
					$(".pop-sign").removeClass("none");
					$(".sign-in").addClass("in-up");
				}else {
					that.signHtml();
				}
			});
	...
	issignimgFn: function() {//当天签到的头像
		...
		if(headimgurl) {
			span.eq(H.sign.$the).append('<i class="sign-myhead" style="background-image:url('+headimgurl+')"><em></em></div>');
		}else {
			span.eq(H.sign.$the).append('<i class="sign-myhead" style="background-image:url(images/avatar.jpg)"><em></em></div>');
		}
		$("#sign"+H.sign.$the).addClass("gold-on");
	},
	
	signBtn: function(auid) {//点击签到保存签到记录
		$("#sign-btn").unbind("click").click(function() {
	...
	closeFn: function() {
		$("#sign-btn-return").click(function() {
			$(".sign-in").removeClass("in-up").addClass("out-down");
			setTimeout(function() {
				$(".pop-sign").addClass("none");
				$(".sign-in").removeClass("out-down");
			},400);
		});
	}
};
