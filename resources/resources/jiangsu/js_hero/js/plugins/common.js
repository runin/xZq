$('body').append('<section id="zhibo" class="dialog-wrapper none"><section class="dialog"><section id="tv"><section class="tv-bg"><img class="tv-font" src="./images/box.png" /><section class="tv-snow"></section><section class="tv-cover none"></section><img class="tv-default" src="./images/icon-tv-content.png" /></section></section></section></section>');
...
var showTips = function(word, pos, timer) {
    if (aniTrue) {
        ...
        var pos = pos || '2',
            timer = timer || 1500;
        $('body').append('<div class="tips none fu-tips"></div>');
        $('.tips').css({
            'position': 'fixed',
            'max-width': '80%',
            'top': '50%',
            ...
            'z-index': '99999999',
            'color': '#FFB05B',
            'text-shadow': '0 0 5px rgba(208,0,0,1)',
            'padding': '22px 43px 10px',
            'margin-left': '-120px',
            'background': 'url(images/no.png?v=201609) no-repeat',
...
$(function() {
    ...
    $.ajax({
        ...
        success : function(data) {
            if (!data.result) location.href = data.redirect;
            share_img = data.si;
            share_title = data.st;
            share_desc = data.sd;
            share_group = data.sgt;
            shareData = {"imgUrl": share_img,"link": share_url,"desc": share_desc,"title": share_title};
            window['shaketv'] && shaketv.wxShare(share_img, share_title, share_desc, getUrl(openid));
            if (follow_shaketv_appid) {
