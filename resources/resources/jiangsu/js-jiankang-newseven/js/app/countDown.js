...
;(function (w,$) {
   ...
   $.fn.countDown =function(obj){
     var defaultParam = {
        getSystemTimeUrl:domain_url+'api/common/timestr',//取系统时间的url
				...
				try{
					for(var i=countIndex,len= p.timeArr.length;i<len;i++){
						...
						if(st<t&&t<et){
							...
							$(that).html(tmp);
							p.atTimeFn && p.atTimeFn(that, p.timeArr[i].index,tmp,o,t);
							break;
						};
						if(t>et){
							countIndex++;
     ...
     o.loadData({url:p.getSystemTimeUrl,commonApiTimeStrHandler :function(data){
