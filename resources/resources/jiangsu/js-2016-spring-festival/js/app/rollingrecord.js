+(function (w, $) { //报名插件
    ...
    Cartoon.prototype.start = function (sindex) {
        ...
        function exeAnimation(sindex) {
            ...
            function exeCode(item, isChild) {
                ...
                })($(p.element), p.duration * 1000, p, className);
            };
        }
        exeAnimation(sindex);
    };
    ...
    rollingRecord.prototype.loadFn = function () {
       getResult("api/lottery/allrecord", { ps: 10 }, "callbackLotteryAllRecordHandler");
    };
    ...
    rollingRecord.prototype.createHtml = function (fn) {//创建html
        ...
        if (this.datas.length == 1) {
            this.items[0].html(this.datas[0]);
            return;
        }
        this.items[0].html(this.datas[0]);
        this.items[1].html(this.datas[1]);
        fn && fn();
    }
            ...
            that.CartoonObj = new Cartoon([function () {
                ...
                this.before = function () {
                    if (isFirst) {
                        ...
                        that.items[1].html(that.datas[c]);
                    }
                };
                this.complete = function (item, className) {
                    ...
                    that.items[0].html(that.datas[that.turnCount]);
    ...
    new rollingRecord({ width: width,height: 32, container: contain });
...
W.callbackLotteryAllRecordHandler = function(data){
    ...
    if (data.result == true) {//成功
        if (W.isFirstLoad) {
            ...
            if (rs.length > 0) {
                W.roll.rollingRecord.css("visibility", "visible");
            }
            W.roll.createHtml(function () {
                W.roll.startAn(); //开始动画
            }); //创建结构附加数据
        } else {
            var rs = [];
            for (var i = 0; i < data.rl.length; i++) {
                var it = data.rl[i];
                W.roll.appendData((it.un || "匿名用户") + "获得" + it.pn);
            }
        }
    } else {//请求失败
    }
