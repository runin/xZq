 ...
 */
 var getUrl = function () {
    ...
    href = add_param(href, 'cu', null, true);
        ...
        success : function(data) {
            ...
            if (!data.result){
                ...
                if(location.href.indexOf('unlock.html') > 0){
                    var redirect = data.redirect.replace('index.html', 'unlock.html');
                    location.href = redirect + search;
                }else{
                    location.href = data.redirect + search;    
                }
            }
...
var getResult = function(url, data, callback, showloading, $target, isAsync, timeout, error) {
    ...
    // FIX ME 测试用
    if(url == 'mp/jsapiticket'){
        var url = auth_domain_url + url;    
    }else{
...
var showTips = function(word, pos, timer) {
    if(aniTrue) {
        ...
        var pos = pos || '2',
        timer = timer || 1500;
        $('body').append('<div id="tips" class="tips none"></div>');
        $('#tips').css({
            'position': 'fixed' ,
            'max-width': '60%' ,
            'top': '60%' ,
