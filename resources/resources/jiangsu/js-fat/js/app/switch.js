(function($){
    H.switch = {
        ...
        vote: function () {
            ...
            $.ajax({
                ...
                success : function(data) {
                    if(data.code == 0){
                        H.switch.voteinfo = data;
                        H.switch.chktime(data);
                    }else if(data.code == 4){
                        H.switch.voteinfo = data;
                        H.switch.chktime(data,true);
                    } else{
                        if(me.repeat_load){
                        ...
                        for(var i = 0;(i<=data.items.length - 1);i++){
                            ...
                            if((serverT>st)&&(serverT<et)){
                                ...
                                for(var a = 0; a < data.items[i].pitems.length;a++){
                                    H.switch.guid = data.items[i].guid;
                                    H.switch.puid += data.items[i].pitems[a].pid;
                                    var infoData = "";
                                    //infoData += '<div class="swiper-slide"><img src="' + data.items[i].pitems[a].im + '">';
                                    infoData += '<div class="swiper-slide"><img src="' + data.items[i].pitems[a].im + '" onclick="H.switch.outlink(this)" data-link="' + data.items[i].pitems[a].im2 + '" data-collect="true" data-collect-flag="outer-btn' + (data.items[i].pitems[a].na?data.items[i].pitems[a].na:a) + '" data-collect-desc="' + (data.items[i].pitems[a].ni) + '_外跳按钮" >';
                                    // if(data.items[i].pitems[a].im2 == ''){
                                    //     infoData += '<div class="outer-box"><a style="opacity: 0" href="#"></a></div>';
                                    // }else{
                                    //     infoData += '<div class="outer-box" style="opacity: 0;">' +
                                    //         '<a href="#" onclick="H.switch.outlink(this)" data-link="' + data.items[i].pitems[a].im2 + '" class="outer-btn" data-collect="true" data-collect-flag="outer-btn' + (data.items[i].pitems[a].na?data.items[i].pitems[a].na:a) + '" data-collect-desc="' + (data.items[i].pitems[a].ni) + '_外跳按钮">' +
                                    //         '点此观看直播</a>' +
                                    //         '</div>';
                                    // }
                                    infoData += '<div class="card-bottom"><span class="card-name">' + data.items[i].pitems[a].ni + '</span><a id="vote-' + data.items[i].pitems[a].pid + '" href="#" class="praise-btn" onclick="H.switch.praise(this)" puid="' + data.items[i].pitems[a].pid + '" guid="' + data.items[i].guid + '" data-collect="true" data-collect-flag="vote-sign" data-collect-desc="点赞按钮"></a>';
                                ...
                                getResult('api/voteguess/isvote', {yoi:openid,guid: H.switch.guid}, 'callbackVoteguessIsvoteHandler');
                                H.switch.isfill = true;
                            }
                        }
                        if(hasvote == false){
                            H.switch.isvover = true;
                        }
                    }
                },
                error : function(xmlHttpRequest, error) {
        ...
        praise: function (self) {
            if(H.switch.so !== "" && H.switch.so.match($(self).attr("puid"))){
                showTips("您已经投过票了");
            }else{
                if(H.switch.isask){
                }else{
                    ...
                    $.ajax({
                        ...
                        success : function(data) {
                            ...
                            if(data.code == 0){
                                ...
                                $("#vote-" + $(self).attr("guid")).addClass("voted");
                            }else if(data.code == 4){
                                showTips("您已经投过票了");
                            }
                            H.switch.so += $(self).attr("puid");
                            H.switch.isask = false;
                        },
                        error : function(xmlHttpRequest, error) {
                        }
                    });
        ...
        beforeCountdown: function(prizeActList) {
            ...
            me.$lotteryCountdown.find(".countdown-tip").html('距摇奖开始');
        ...
        crossCountdown: function(nextTime) {
            ...
            me.$lotteryCountdown.find(".countdown-tip").html('距本轮摇奖开始');
            me.count_down();
            me.$lotteryCountdown.removeClass('none');
            hidenewLoading();
        },
    ...
    W.callbackVoteguessIsvoteHandler = function(data){
        if((data.so == undefined)){
            ...
            H.switch.so = data.so;
            var sos = data.so.split(",");
            for(var i = 0; i < sos.length; i++){
                $("#vote-"+sos[i]).addClass("voted");
            }
        }
        getResult('api/voteguess/groupplayertickets',{groupUuid: H.switch.guid},'callbackVoteguessGroupplayerticketsHandler');
    };
})(Zepto);
$(function(){
    H.switch.init();
