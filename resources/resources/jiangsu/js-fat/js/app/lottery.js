(function($) {
    ...
    W.commonApiPromotionHandler = function(data){
        if (data.code == 0 && data.desc && data.url) {
            ...
            var index = localStorage.getItem("jsfat_link_index")?(localStorage.getItem("jsfat_link_index")*1):0;
            if(index >= url_list.length){
                index = 0;
            }
            $('#ddtj').attr('data-collect-flag', "jsfat-link-btn-"+index).attr("data-collect-desc","摇奖页-吸粉链接-"+index);
            $('.ddtj').text(desc_list[index] || '');
            $('#ddtj').click(function(){
                location.href = url_list[index];
                localStorage.setItem("jsfat_link_index", ++index);
            });
