...
(function($) {
		...
		tpl: function(data) {
			...
			for (var i = 0, len = item.length; i < len; i++) {
				...
				t._('<li data-uuid = "'+ item[i].uid +'">')
					...
					._('<div>')
						._('<label class="zan '+isZan+'" data-collect="true" data-collect-flag="gz-city-comments-zan" data-collect-desc="点赞" >'+ item[i].pc +'</label>')
						._('<p>'+ (item[i].na || '匿名用户') +'</p>')
						._('<p class="all-con" id="all-con'+ me.item_index +'">')
							._('<span>'+ item[i].co +'</span>')
						._('</p>')
						._('<a class="show-all none" id="show-all'+ me.item_index +'" data-collect="true" data-collect-flag="gz-city-comments-show" data-collect-desc="评论收缩显示" >^显示全部</a>')
					._('</div>')
					._('</li>');
				++ me.item_index;
			}
		...
		if(data.code == 0){
			...
			if(com.attrs != null && com.attrs.length != 0){
				...
				for (var i = 0, len = result.length; i < len; i++) {
                    $sx_ul = $('#sup');
                    if(result[i].ai != ""){
                        t._('<a class="btn support" style="background:'+result[i].acl+'" data-uuid = "'+ result[i].au +'"><i class="hand" style="background: url('+result[i].ai+') no-repeat right center;background-size:17px auto"></i><label>'+result[i].av+'</label></a>');
                    }else{
                ...
                for (var i = 0, len = result.length; i < len; i++) {
                    ...
                    t._('<p>')
                        ._('<label><input class="radio" type="radio" '+select+'>'+ result[i].av +'</label>')
	...
	W.supportHandler = function(data){
        if(data.code == 0){
            ...
            for (var i = 0, len = result.length; i < len; i++) {
                ...
                var select = result[i].au == H.comments.attrUuid ? 'checked="checked"' : '';
                t._('<p>')
                    ._('<label><input class="radio" type="radio" '+select+'>'+ result[i].av +'</label>')
                    ._('<i class="support-pro"><span style="width:'+(percent-2)+'%;background:'+result[i].acl+'"></span></i>')
                    ._('<span class="lv">'+percent+'%</span>')
	...
	W.callbackCommentsSave = function(data){
		if(data.code == 0 ){
			...
			._('<img src="'+ headImg +'"/>')
			._('<div>')
				._('<label class="zan-'+data.uid+'" class="zan" data-collect="true" data-collect-flag="gz-city-comments-zan" data-collect-desc="点赞" >'+ 0 +'</label>')
				._('<p>'+ (nickname || '匿名用户') +'</p>')
				._('<p class="all-con" id="all-con'+ data.uid +'">')
					._('<span>'+ $("#comments-info").val() +'</span>')
				._('</p>')
				._('<a class="show-all none" id="show-all'+ data.uid +'" data-collect="true" data-collect-flag="gz-city-comments-show" data-collect-desc="评论收缩显示">^显示全部</a>')
			._('</div>')
			._('</li>');
