...
$(function() {
	...
	$.ajax({
		...
		success : function(data) {
			if (!data.result){
				// 标识码cb41faa22e731e9b的传参
				var currentUrl = '';
				if (uuid) {
					currentUrl = "?uuid=" + uuid;
				} else {
					currentUrl += '';
				};

				if (from && currentUrl.indexOf('?') >= 0) {
					currentUrl += "&from=" + from;
				} else if (from && currentUrl.indexOf('?') < 0) {
					currentUrl += "?from=" + from;
				} else {
					currentUrl += '';
				};

				if (gefrom && currentUrl.indexOf('?') >= 0) {
					currentUrl += "&gefrom=" + gefrom;
				} else if (gefrom && currentUrl.indexOf('?') < 0) {
					currentUrl += "?gefrom=" + gefrom;
				} else {
					currentUrl += '';
				};
				...
				// 跳转到新的领奖页面
				var lingjiangUrl = data.redirect;
				location.href = lingjiangUrl.replace('/index.html','/lingjiang.html') + encodeURIComponent(currentUrl);
			};
			share_img = data.si;
			share_title = data.st;
			share_desc = data.sd;
