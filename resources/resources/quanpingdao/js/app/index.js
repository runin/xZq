	...
	W.callbackApiRedProlistHandler = function(data) {//首页滚动图
		...
		if(data && data.code == 0){
			...
			for(var k=0, leg=items.length; k<leg; k++) {
				...
				t._('<div class="order-box">')
				t._('<div class="dowmtime-con" id="dowmtime'+k+'"></div>');
				t._('<p class="btnbox'+k+'">');
				t._('<a href="javascript:void(0)" class="btn-order1 bespeak'+k+' none">预约节目</a>');
				t._('<a href="'+items[k].gourl+'" id="interact'+k+'" class="btn-order1 none">进入互动</a>');
				t._('</p>');
				t._('</div>')
				t._('</div>');
