(function (W) {
    ...
    Authorize.prototype.userinfoInit = function(fn){
        if(!nickname){
            ...
            // FIX ME
            $(".head-url").css("background-image","url('"+headimgurl+"')");
    ...
    Authorize.prototype.authorizeUserInfo = function () {
        ...
        var redirect_uri = auth_url + "api/mp/auth/saas_snsapi_userinfo";
        var param = this.getParam();
        window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + mpappid + "&redirect_uri=" + encodeURIComponent(redirect_uri+"?callBackPage="+location.href+"&param="+param)+ "&response_type=code&scope=" + scope +  "&state=" + mpappid + "#wechat_redirect";
    };

    Authorize.prototype.authorizeBase = function () {
        var scope = "snsapi_base";
        var redirect_uri = auth_url + "api/mp/auth/saas_snsapi_base";
        var param = this.getParam();
        window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + auth_mpappid + "&redirect_uri=" + encodeURIComponent(redirect_uri+"?callBackPage="+location.href+"&param="+param)+ "&response_type=code&scope=" + scope +  "&state=" + auth_mpappid + "#wechat_redirect";
    };
    ...
    var get_info = function(code) {
        $.ajax({
            type : 'GET',
            async : false,
            url : auth_url + 'shaketv/yaotv/userinfo',
