...
function getList(showloading) {
	getResult("mall/item/newlist", {cuid: channelUuid, page: page, pageSize: pageSize,programType:programType}, 'callbackStationMall', showloading);
}
...
function callbackStationMall(data) {
	...
	if (data.code == 0) {
		...
		for (var i = 0; i < len; i ++) {
			...
			t._('<li>')
				._('<a href="'+ url +'" data-collect="true" data-collect-flag="sy-evening-paper-main-mall" data-collect-desc="积分商城 '+ items[i].n +'">')
				._('<img src="'+ items[i].is +'" />')
				._('<div class="right">')
				._(items[i].bd?'<h2>'+ items[i].bd +'</h2>':'')
				._(items[i].sn?'<h2>'+ items[i].sn +'</h2>':'')
				._('<p>兑换积分：'+ items[i].ip +'</p>')
				._('<p>价格：' + '<span class="price">￥' + (Math.round((items[i].mp || 0)) / 100) + '</span></p>')
				._('</div>')
				._('</a>')
				._('</li>');
		}
		...
		for (var i = 0; i < len; i ++) {
			if(i%2==0){
				//$('#list li').eq(i).css("background","rgba(255, 255, 255, 0.4)");
			}
		}
	} else {
