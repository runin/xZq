	...
	H.dialog = {
		...
		init: function() {
			...
			this.$container.delegate('.btn-close', 'click', function(e) {
				e.preventDefault();
				$('.lottery-dialog').removeClass('bounceInDown').addClass('bounceOutUp');
				setTimeout(function(){
					$('.btn-close').closest('.modal').addClass('none');
					hideLottery();
				},1500);
			});
		},
		...
		fudai: {
			...
			tpl: function() {
				...
				t._('<section class="modal" id="fudai-dialog">')
					._('<div class="fudai" data-collect="true" data-collect-flag="sy-news-fudaidialog-openbtn" data-collect-desc="福袋弹层-打开按钮">')
						._('<img class="aureole aureole-an" src="images/aureole.png">')
						._('<img class="lh cjani2" src="images/lihe.png">')
						._('<p>点击礼盒抽大奖>></p>')
					._('</div>')
				._('</section>');
				return t.toString();
		...
		lottery: {
			...
			open: function(data) {
				...
				hideLoading($dialog);
				H.dialog.open.call(this);
				$('.lottery-dialog').addClass('bounceInDown');
				if (!$dialog) {
					this.event();
				}
				...
				if (data != null && data != '') {
					if (data.result) {
						if (data.pt === 1) { //实物奖品
							...
							this.$dialog.find('.awarded-tip').html(data.aw || '');
				...
				t._('<section class="modal" id="lottery-dialog">')
					...
					._('<div class="lottery-dialog animated">')
						...
						._('<div class="award-none none">')
							._('<img src="./images/not-tsy.png">')
							._('<h5>您与大奖就这么擦肩而过了,<br>不要灰心，继续加油！</h5>')
