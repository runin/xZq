...
function spellCurrentHtml(data){
    ...
    if(data.sinfo.tlist){
        $.each(data.sinfo.tlist,function(i,item){
            resultStr += '<li class="ques-lied" id="'+item.tuuid+'"><i></i><label>'
                + item.sv + '</label></li>';
        });
    }
    resultStr +='</ul>';
...
function spellpercentHtml(data){
    ...
    if(data.sinfo.tlist){
        ...
        $.each(data.sinfo.tlist,function(i,item){
            ...
            resultStr += '<li><p>'
                + item.sv +'</p><label class="percent">'+ checkedResult + '</label ><div><p class="plan"></p> <p class="plan-bg"></p></div></li>';
        });
    }
    resultStr +='</ul>';
    return resultStr;
...
function callbackSurveyCurrentHander(data) {
    ...
    if (data.code==1) {
        ...
        var win = $('.plan').width();
        $.each(data.sinfo.tlist,function(i,item){
            console.log(item.ac/data.count);
            checkedResult = (item.ac/data.count *win) + 'px';
...
function callbackSurveySaveHander(data){
    if (data.result) {
        showTips("谢谢您参与调查，下面进行抽奖");
        var url = "yao.html?surveyInfoUuid=" + surveyInfoUuid;
        if (gefrom != null && gefrom != '') {
            url = url + "&gefrom=" + gefrom;
        }
        location.href = url;
//      $('.red-fixed').removeClass('none');
//      $('.red-fixed .open').removeClass('none');
    } else {
        $("#submitBtn").html(data.message);
        $("#submitBtn").attr('class','subed');
...
window.onload = function(){
    $('.red-fixed').css('height',document.body.scrollHeight);
};