(function($) {
	H.answer = {
		...
		fill: function(data) {
			...
			this.$article.css({'background':'url('+ data.bm +') no-repeat top center','background-size':'cover'});
					...
					t._('<li data-qcode="'+ qitems[i].qcode +'" data-ruid="'+ qitems[i].ruid +'" data-stime="'+ timestamp(qitems[i].qst) +'" data-etime="'+ timestamp(qitems[i].qet) +'" id="question-'+ qitems[i].quid +'" data-quid="'+ qitems[i].quid +'">')
						._('<dl>')
							...
							for (var j = 0, jlen = aitems.length; j < jlen; j ++) {
								t._('<dd><a href="#" class="q-item" data-auid="'+ aitems[j].auid +'" data-collect="true" data-collect-flag="sy-tvshow-answer-answer" data-collect-desc="答题页面-答案按钮">'+ aitems[j].at +'</a></dd>');
							}
                ...
                $.ajax({
                    ...
                    success : function(data) {
                        ...
                        if (data.code == 0) {
                            ...
                            barrage.appendMsg('<div class="c_head_img"><img class="isme" class="c_head_img_img" src="' + h + '" /></div>'+comment);
                ...
                $.ajax({
                    ...
                    success : function(data) {
                        ...
                        if (data.code == 0) {
							...
							var h= headimgurl ? headimgurl + '/' + yao_avatar_size : './images/danmu-head.png';
			                barrage.appendMsg('<div class="c_head_img"><img class="c_head_img_img" src="' + h + '" />'+'<div class="funnyshow"><img src="./images/funny/' + nfunny + '.png" border="0" /></div></div>');
