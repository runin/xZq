...
(function($) {
    ...
    W.callbackArticledetailListHandler = function(data){
        if(data == undefined){
            ...
            if(data.code == 0){
                hidenewLoading();
                $(".top-btn").eq(0).before('<img class="top-ad" src="' + (data.arts[1].img?data.arts[1].img:"images/tv-info.png").toString() + '" />');
                //$(".fail-bg").css({"background":'url(' + (data.arts[3].img?data.arts[3].img:"images/tv-info.png").toString() + ') no-repeat',"background-size":"100% 100%"});
