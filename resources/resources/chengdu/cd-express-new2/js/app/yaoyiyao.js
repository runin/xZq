...
(function($) {
    H.lottery = {
        ...
        fill: function(data) {
            ...
            H.dialog.isGetNew = true;
            if (data.pt == 7 || data.pt == 9 || data.pt == 4) {
                //卡券，外链
    ...
    W.callbackArticledetailListHandler = function(data){
        if(data == undefined){
            ...
            if(data.code == 0){
                hidenewLoading();
                $(".top-btn").eq(0).before('<img class="top-ad" src="' + (data.arts[1].img?data.arts[1].img:"images/thank-bg.png").toString() + '" />');
                //$(".fail-bg").css({"background":'url(' + (data.arts[3].img?data.arts[3].img:"images/thank-bg.png").toString() + ') no-repeat',"background-size":"100% 100%"});
                $(".top-ad").on("click", function () {
                    window.location.href = data.arts[1].gu;
                });
            }else if(data.code == 1){
