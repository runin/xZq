			...
			tpl: function() {
				...
				t._('<section class="modal modal-guide" id="guide-dialog">')
					._('<div class="dialog guide-dialog relocated">')
						...
						._('<div class="guide-content"><p><i></i>1.打开电视，锁定深圳卫视</p>')
						._('<p><i></i>2.打开微信，进入摇一摇(电视)</p>')
						._('<p><i></i>3.对着电视摇一摇</p>')
		...
		fudai: {
			...
			tpl: function() {
				...
				t._('<section class="modal" id="fudai-dialog">')
					...
					._('<div class="fudai" data-collect="true" data-collect-flag="rankdialog-clickbtn" data-collect-desc="领奖弹层-点击按钮"><div class="hand"></div><div class="fudai-round"></div class="openit"></div>')
				._('</section>');
				return t.toString();
			}
		},
				...
				if (data != null && data != '') {
					if (data.result) {
						if (data.pt === 1) { //实物奖品
							...
							this.$dialog.find('img').attr('src', (data.pi || ''));
							this.$dialog.addClass(this.LOTTERIED_CLS);
                    	 	recordUserPage(openid, "《拥抱星星的月亮》红包领奖页", '');
							this.$dialog.find('.award-win').removeClass('none');
