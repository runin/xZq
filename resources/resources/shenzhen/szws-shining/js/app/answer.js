					...
					t._('<li data-qcode="'+ qitems[i].qcode +'" data-ruid="'+ qitems[i].ruid +'" data-stime="'+ timestamp(qitems[i].qst) +'" data-etime="'+ timestamp(qitems[i].qet) +'" id="question-'+ qitems[i].quid +'" data-quid="'+ qitems[i].quid +'">')
						._('<dl>')
							...
							for (var j = 0, jlen = aitems.length; j < jlen; j ++) {
								t._('<dd><a href="#" class="q-item" data-auid="'+ aitems[j].auid +'" data-collect="true" data-collect-flag="szws-shining-answer-answer" data-collect-desc="答题页面-答案按钮">'+ aitems[j].at +'</a></dd>');
							}
                ...
                success : function(data) {
                    ...
                    for (var i = 0, len = items.length; i < len; i ++) {
                    	if ((items[i].co).indexOf(umoReg) >= 0) {
                    		...
                    		var nfunny = funny.replace('/:','');
			                barrage.appendMsg('<img class="ismenow" src="' + (items[i].hu ? (items[i].hu + "/" + yao_avatar_size) : "./images/danmu-head.jpg") + '" />'+'<div class="funnyshow"><img src="./images/funny/' + nfunny + '.png" border="0" /></div>');
                    	} else {
                    		barrage.pushMsg("<img src='" + (items[i].hu ? (items[i].hu + '/' + yao_avatar_size) : './images/danmu-head.jpg') + " '/>"+items[i].co);
                    	};
                    }
                }
            });
