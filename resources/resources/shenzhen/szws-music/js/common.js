...
var getResult = function (options) {
    ...
    if (is_dev || (localStorage && localStorage.dev)) {
        ...
        typeof opts.success === 'function' && opts.success(window[opts.jsonpCallback + 'Data']);
        return;
    }

    var $bindUI = $(opts.bindUI);
    if (opts.bindUI && $bindUI.length > 0) {
        if ($bindUI.hasClass(opts.bindCls)) {
            return;
        }
    }
    $bindUI.addClass(opts.bindCls);

    if (opts.loading) {
        showLoading(opts.$loadingTarget);
    }

    $.ajax({
        type: 'GET',
        async: opts.isAsync,
        url: domain_url + opts.url,
        data: opts.data,
        dataType: "jsonp",
        jsonpCallback: opts.jsonpCallback,
        complete: function () {
            $bindUI.removeClass(opts.bindCls);
            if (opts.loading) {
                hideLoading(opts.$loadingTarget);
            }
        },
...
var hex_sha1 = function (u) {
    function l(b) {
        return p(o(q(b), b.length * 8))
    }
    function s(h) {
        var f = "",
        e;
        for (var g in h) {
            e = h.charCodeAt(g),
            f += (e >> 4 & 15).toString(16) + (e & 15).toString(16)
        }
        return f
    }
    function r(h) {
        ...
        while (++i < h.length) {
            b = h.charCodeAt(i),
            g = i + 1 < h.length ? h.charCodeAt(i + 1) : 0,
            55296 <= b && b <= 56319 && 56320 <= g && g <= 57343 && (b = 65536 + ((b & 1023) << 10) + (g & 1023), i++),
            b <= 127 ? j += u(b) : b <= 2047 ? j += u(192 | b >> 6 & 31, 128 | b & 63) : b <= 65535 ? j += u(224 | b >> 12 & 15, 128 | b >> 6 & 63, 128 | b & 63) : b <= 2097151 && (j += u(240 | b >> 18 & 7, 128 | b >> 12 & 63, 128 | b >> 6 & 63, 128 | b & 63))
        }
        return j
    }
    function q(f) {
        var d = [];
        for (var e = 0; e < f.length * 8; e += 8) {
            d[e >> 5] |= (f.charCodeAt(e / 8) & 255) << 24 - e % 32
        }
        return d
    }
    function p(e) {
        var f = "";
        for (var b = 0; b < e.length * 32; b += 8) {
            f += u(e[b >> 5] >> 24 - b % 32 & 255)
        }
        return f
    }
    function o(x, y) {
        ...
        for (var w = 0; w < x.length; w += 16) {
            ...
            I = z;
            for (var F = 0; F < 80; F++) {
                F < 16 ? E[F] = x[w + F] : E[F] = t(E[F - 3] ^ E[F - 8] ^ E[F - 14] ^ E[F - 16], 1);
                var G = v(v(t(D, 5), n(F, C, B, A)), v(v(z, E[F]), m(F)));
                z = A,
                A = B,
                B = t(C, 30),
                C = D,
    ...
    function n(g, f, e, h) {
        return g < 20 ? f & e | ~f & h : g < 40 ? f ^ e ^ h : g < 60 ? f & e | f & h | e & h : f ^ e ^ h
    }
    function m(b) {
        return b < 20 ? 1518500249 : b < 40 ? 1859775393 : b < 60 ? -1894007588 : -899497514
    }
    function v(e, h) {
        var f = (e & 65535) + (h & 65535),
        g = (e >> 16) + (h >> 16) + (f >> 16);
        return g << 16 | f & 65535
    }
    function t(d, c) {
