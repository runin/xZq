    ...
    if(data.code == 0){
        for(var i = 0;(i<=data.items.length - 1);i++){
            ...
            if((serverT>st)&&(serverT<et)){
                ...
                $.ajax({
                    type:"GET",
                    url:domain_url+"api/sign/issign",
                    dataType:"jsonp",
                    jsonp: "callback",
...
function callbackSignIsHandler(data){
    if(data == undefined){
        ...
        hidenewLoading();
        if(data.result == true){
            qiandao.css({"background-color":"#946f00","display":"block"}).on("click", function () {
                showTips("今日已经签到");
            });
            ...
            qiandao.css({"display":"block"}).on("click", function () {
                ...
                $.ajax({
                    type:"GET",
                    url:domain_url+"api/sign/signed",
                    dataType:"jsonp",
                    jsonp: "callback",
    ...
    if(data == undefined){
        ...
        if(data.code == 0){
            ...
            if(data.signVal == undefined){
                ...
                qiandao.css({"background-color":"#946f00","display":"block"}).off();
            }
        }else if(data.code == 2){
