(function($) {
    H.lottery = {
        ...
        beforeCountdown: function(prizeActList) {
            ...
            hidenewLoading();
            if($("body").attr("data-type") != "lottery"){
                me.$goLottery.addClass("none");
                $("#lottery-countdown").removeClass("opacity0");
            }
        },
        // 摇奖结束倒计时
        nowCountdown: function(prizeActList){
            ...
            if($("body").attr("data-type") != "lottery"){
                H.dialog.current_lottery.open();
                $("#lottery-countdown").addClass("opacity0");
                me.$goLottery.removeClass("none").find("span").text("去摇奖");
            }
        },
        // 跨天摇奖开启倒计时
        ...
        count_down: function() {
            ...
            me.$lotteryCountdown.find('.detail-countdown').each(function() {//摇奖页倒计时
                $(this).countDown({
                    etpl : '<label>%H%</label>' + '<span>'+ H +'</span>'+'<label>%M%</label>' + '<span>'+ M +'</span>' + '<label>%S%</label>' + S, // 还有...结束
                    stpl : '<label>%H%</label>' + '<span>'+ H +'</span>'+'<label>%M%</label>' + '<span>'+ M +'</span>' + '<label>%S%</label>' + S, // 还有...开始
                    sdtpl: '',
                    otpl: '',
                    otCallback: function() {
                        if(me.canJump){
        ...
        change: function() {
            ...
            hidenewLoading();
            if($("body").attr("data-type") != "lottery"){
                me.$goLottery.find("span").text("本期摇奖已结束，下期再战！");
                $("#lottery-countdown").addClass("opacity0");
                me.$goLottery.addClass("none");
                $("#lottery-countdown").removeClass("opacity0");
            }
        ...
        if(data.result){
            ...
            if(list && list.length > 0){
                ...
                for(var i = 0 ; i < list.length; i++){
                    ...
                    con += "<li><i/></i><span>" + username + "中了 <label>" + list[i].pn + "</label></label></span></li>";
                }
