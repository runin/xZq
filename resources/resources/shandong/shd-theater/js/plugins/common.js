...
var toUrl = function(url) {
    ...
    var delay = Math.ceil(500*Math.random() + 50);
...
var cookie4toUrl = function(page) { //cookie中的jumpNum值，0为正常，当值超过3时页面跳转至yaoyiyao.html
    shownewLoading(null, '请稍后...');
    var exp = new Date(), jumpTime = 3;
    exp.setTime(exp.getTime() + 60*1000);
    if ($.fn.cookie('jumpNum')) {
        if ($.fn.cookie('jumpNum') >= 0 && $.fn.cookie('jumpNum') < jumpTime) {
            var jumpNum = parseInt($.fn.cookie('jumpNum')) + 1;
            $.fn.cookie('jumpNum', jumpNum, {expires: exp});
            toUrl(page);
        } else if($.fn.cookie('jumpNum') >= jumpTime) {
...
function anictrl(o){
    if(o instanceof Array){
        for(var i=0;i<o.length;i++){
            if(o[i].obj.length>1){
                for(var a=0;a<o[i].obj.length;a++){
                    execAni(o[i],o[i].obj[a]);
                }
            }else{
                execAni(o[i]);
            }
    ...
    function execAni (self,obj) {
        var aniObj = obj,ani = self.ani + " ",during = self.during,rdt = self.rdt;
        if(typeof rdt !== "undefined" && rdt !== "" && rdt.length == 2){
            ani += ((rdt[0] + rdt[1]*Math.random()) * 0.001 + "s").toString();
        }
        if(typeof self.during == "undefined" || self.during == ""){during = "linear"}
        if(typeof self.delay == "undefined" || self.delay == "" || self.delay<5){self.delay = 5}
        if(typeof obj == "undefined"){if(self.obj instanceof Array){aniObj = self.obj[0]}else{aniObj = self.obj}}
        if(aniObj.css("-webkit-animation").indexOf("0s") !== -1){
            setTimeout(function () {
                aniObj.css({"-webkit-animation": ani.toString(),"-webkit-animation-timing-function": during.toString()}).one("webkitAnimationEnd", function () {
                    aniObj.css({"-webkit-animation":"","animation":""}).off("webkitAnimationEnd");
                    if(typeof self.efunc !== "undefined" && self !== ""){self.efunc()}
                });
            },self.delay)
        }
    }
