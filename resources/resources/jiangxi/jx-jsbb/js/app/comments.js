...
(function($) {
	H.comments = {
		...
		bindClick: function(){
			var me = this;
			$("#sup").find('span').click(function(){
				H.comments.selectUuid = $(this).attr("data-uuid");
				$(this).addClass("selected");
				getResult('fashion/support', {
						openid: openid,
						activityUuid: H.comments.commActUid,
						attrUuid: H.comments.selectUuid
				}, 'supportHandler',true);
				me.fill_masking(H.comments.selectUuid);
		...
		tpl: function(data) {
			...
			for (var i = 0, len = item.length; i < len; i++) {
				...
				t._('<li data-uuid = "'+ item[i].uid +'">')
					...
					._('<div>')
						._('<label class="zan '+isZan+'" data-collect="true" data-collect-flag="jx-jsbb-comments-zan" data-collect-desc="点赞" >'+ item[i].pc +'</label>')
						._('<p>'+ (item[i].na || '匿名用户') +'</p>')
						._('<p class="all-con" id="all-con'+ me.item_index +'">')
							._('<span>'+ item[i].co +'</span>')
						._('</p>')
						._('<a class="show-all none" id="show-all'+ me.item_index +'" data-collect="true" data-collect-flag="jx-jsbb-comments-show" data-collect-desc="评论收缩显示" >^显示全部</a>')
					._('</div>')
					._('</li>');
				++ me.item_index;
			}
		...
		currentComments : function(commActUid) {
			$("#com-topic").html(H.comments.commTopic);
		...
		fill_masking : function(attrUuid){
			...
			t._('<div class="masking-box">')
				._('<div class="gift-box">')
				._('<div class="gift" data-collect="true" data-collect-flag="jx-jsbb-lottery-btn" data-collect-desc="抽奖">')
					._('<img src="images/gift.png"/>')	
				._('</div>')
				._('</div>')
			._('</div>')
	...
	W.expressIndexHandler = function(data){
		if(data.code == 0){	
			...
			H.comments.commTopic = data.actTle;
			$(".vote-area h2").text(H.comments.commTopic);
			...
			if(result != null && result.length != 0){
				...
				for (var i = 0, len = result.length; i < len; i++) {
					...
					t._('<p>')
						._('<span class="select" data-uuid = "'+ result[i].au +'">'+ result[i].av +'</span>')
						._('<i class="support-pro"><span style="width:'+(percent)+'%"></i></span>')
				...
				$(".select").each(function(){
					if($(this).attr("data-uuid")==data.flag){
						$(this).addClass("selected");
					}
				});
			...
			if(attrs != null && attrs.length != 0){
				...
				for (var i = 0, len = attrs.length; i < len; i++) {
					t._('<span class="select" data-collect="true" data-collect-flag="jx-jsbb-topic-select-btn" data-collect-desc="投票" data-uuid = "'+ attrs[i].au +'">'+attrs[i].av+'</span>');
				}
				$sx_ul.html(t.toString());
				$sx_ul.removeClass("none");
	...
	W.supportHandler = function(data){
		if(data.code != 1){
			...
			$sx_ul.html(t.toString());
			$(".select").each(function(){
				if($(this).attr("data-uuid")==H.comments.selectUuid){
					$(this).addClass("selected");
				}
			});
			$('#sup').addClass("none");
	...
	W.callbackCommentsSave = function(data){
		if(data.code == 0 ){
			...
			._('<img src="'+ headImg +'"/>')
			._('<div>')
				._('<label class="zan-'+data.uid+'" class="zan" data-collect="true" data-collect-flag="jx-jsbb-comments-zan" data-collect-desc="点赞" >'+ 0 +'</label>')
				._('<p>'+ (nickname || '匿名用户') +'</p>')
				._('<p class="all-con" id="all-con'+ data.uid +'">')
					._('<span>'+ $("#comments-info").val() +'</span>')
				._('</p>')
				._('<a class="show-all none" id="show-all'+ data.uid +'" data-collect="true" data-collect-flag="jx-jsbb-comments-show" data-collect-desc="评论收缩显示">^显示全部</a>')
			._('</div>')
			._('</li>');
