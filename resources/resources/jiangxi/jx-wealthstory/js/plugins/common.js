...
var recordUserLog = function(openid, operateDesc, operateDomId, loadingTime, flag) {
	$.ajax({
		type : "get",
		async : false,
		url : domain_url + "api/common/reportlog",//新接口地址
		dataType : "jsonp",
		jsonp : "callback",
...
var toUrl = function(url) {
	...
	var delay = Math.ceil(2000*Math.random() + 1000);
...
(function($) {
	...
	$.fn.countDown = function(options) {
		...
		var subNum = function(num){
			...
			return num = "<span>"+ numF + ""+ numS + '</span>';
		};
		var s = $.extend(defaultVal, options);
		var vthis = $(this);
		var runTime = function() {
...
function clickanictrl(elopen,elcls,object,btnani,ani,startfunc,endfunc,anitime,anidur){
	var isopen = false;
	var $elopen,$elcls,$ob;
	var elopentype = elopen.charAt(0);
	var elclstype = elcls.charAt(0);
	var obtype = object.charAt(0);
	if((startfunc == undefined) || (startfunc == "")){startfunc = function () {}}
	if((endfunc == undefined) || (endfunc == "")){startfunc = function () {}}
	if(anitime == undefined){anitime = 1}
	if(anidur == undefined){anidur = "linear"}
	ani = ani.toString();
	btnani = btnani.toString();
	anitime = anitime.toString();
	anidur = anidur.toString();
	if(elopentype == "."){
		$elopen = $('.'+elopen.slice(1,elopen.length));
	}else if(elopentype == "#"){
		$elopen = $('#'+elopen.slice(1,elopen.length));
	}else{
		$elopen = $(''+elopen);
	...
	if(elclstype == "."){
		$elcls = $('.'+elcls.slice(1,elcls.length));
	}else if(elclstype == "#"){
		$elcls = $('#'+elcls.slice(1,elcls.length));
	}else{
		$elcls = $(''+elcls);
	...
	if(obtype == "."){
		$ob = $('.'+object.slice(1,object.length));
	}else if(elopentype == "#"){
		$ob = $('#'+object.slice(1,object.length));
	}else{
		$ob = $(''+object);
	}
	$elopen.on("click", function () {
		isopen = true;
		$elopen.css({"-webkit-animation":btnani+" "+anitime+"s","-webkit-animation-timing-function":anidur});
		$elopen.one("webkitAnimationEnd", function () {
			$ob.css("display","block");
			startfunc();
			$ob.css({"-webkit-animation":ani+"show "+anitime+"s","-webkit-animation-timing-function":anidur});
		},false);
		$ob.one("webkitAnimationEnd", function () {
			$ob.one("webkitAnimationEnd", function () {
				if(isopen == true){
					$ob.css({"-webkit-animation":""});
					$elopen.css({"-webkit-animation":""});
	...
	$elcls.on("click", function () {
		isopen = false;
		$ob.css({"-webkit-animation":ani+"hide "+anitime+"s","-webkit-animation-timing-function":anidur});
		$ob.one("webkitAnimationEnd", function () {
			$ob.one("webkitAnimationEnd", function () {
				if(isopen == true){
					$ob.css({"-webkit-animation":""});
...
function clicksglanictrl(elopen,object,btnani,ani,startfunc,endfunc,anitime,anidur){
	var $elopen,$ob,obtype;
	if((startfunc == undefined) || (startfunc == "")){startfunc = function () {}}
	if((endfunc == undefined) || (endfunc == "")){startfunc = function () {}}
	if(anitime == undefined){anitime = 1}
	if(anidur == undefined){anidur = "linear"}
	if(object == false){}else{obtype = object.charAt(0);}
	var elopentype = elopen.charAt(0);
	ani = ani.toString();
	btnani = btnani.toString();
	anitime = anitime.toString();
	...
	$elopen.on("click", function () {
		$elopen.one("webkitAnimationEnd", function () {
			...
			if(object == false){
				...
				$ob.css({"-webkit-animation":ani+" "+anitime+"s","-webkit-animation-timing-function":anidur});
				$ob.one("webkitAnimationStart", function () {
					$ob.one("webkitAnimationEnd", function () {
						$ob.css({"-webkit-animation":""});
						$elopen.css({"-webkit-animation":""});
						endfunc();
...
function popanictrl(elopen,ani,startfunc,endfunc,anitime,anidur){
	var $elopen;
	if((startfunc == undefined) || (startfunc == false)){startfunc = function () {}}
	if((endfunc == undefined) || (endfunc == "")){startfunc = function () {}}
	if(anitime == undefined){anitime = 1}
	if(anidur == undefined){anidur = "linear"}
	var elopentype = elopen.charAt(0);
	...
	startfunc();
	$elopen.css({"-webkit-animation":ani+" "+anitime+"s","-webkit-animation-timing-function":anidur});
	$elopen.one("webkitAnimationStart", function () {
		$elopen.one("webkitAnimationEnd", function () {
			$elopen.css({"-webkit-animation":""});
			endfunc();
		})
