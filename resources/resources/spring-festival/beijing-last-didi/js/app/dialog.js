(function($) {
	...
	H.dialog = {
			...
			tpl: function() {
				...
				t._('<section class="modal modal-tips" id="confirm-dialog">')
					._('<div class="dialog tips-dialog confirm-dialog relocated">')
						...
						._('<a href="#" class="btn btn-now">确定</a>')
					._('</div>')
				._('</section>');
				return t.toString();
			}
		...
		didi: {
			...
			event: function() {
				var me = this;
				this.$dialog.find('.btn-tomobile').click(function(e) {
					e.preventDefault();
					me.close();
					$('#mobile').focus();
				});
				this.$dialog.find('.btn-tocard').click(function(e) {
					e.preventDefault();
					me.close();
			...
			tpl: function() {
				var t = simpleTpl();
				t._('<section class="modal modal-tips" id="didi-dialog">')
					._('<div class="dialog tips-dialog didi-dialog relocated">')
						._('<div class="content">您的手机号码尚未填写，滴滴红包领取需提供手机号码，否则视为放弃</div>')
						._('<a href="#" class="btn btn-tomobile">去填手机号码</a>')
						._('<a href="#" class="btn btn-tocard">去发明星贺卡</a>')
					._('</div>')
				._('</section>');
				return t.toString();
			}
