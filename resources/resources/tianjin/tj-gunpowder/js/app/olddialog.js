			...
			tpl: function() {
				...
				t._('<section class="modal modal-guide" id="guide-dialog">')
					._('<div class="dialog guide-dialog relocated">')
						...
						._('<div class="guide-content"><p><i>1</i>打开电视，锁定天津广播电视台</p>')
			...
			open: function(data) {
				...
				if(data.pt == 7){
					...
					document.getElementById('aw').onload = function () {
						...
						$(".lottery-dialog").css({
							'height': inheight+"px!important",
							'top': (winH - inheight) * 0.5+"px!important"
						});
					};
				}else if(data.pt == 4){
					document.getElementById('aw').onload = function () {
						inheight = ($(".award-img").height() + 35) + $(".btn-award").height() + (winH * 0.3);
						if(inheight >= winH){
							$(".award-img img").css("height","60px");
							inheight = ($(".award-img").height() + 35) + $(".btn-award").height()  + (winH * 0.3);
						}
					...
					document.getElementById('imgnone').onload = function () {
						inheight = ($(".btn-close").height() + $("#imgnone").height() + 35);
						if(inheight >= winH){
							$(".award-img img").css("height","60px");
							inheight = ($(".contact").height() + 95) ;
						}
				...
				if (data != null && data != '') {
					if (data.result) {
						if (data.pt === 1) { //实物奖品
							...
							$(".ma").css("width",($(window).width() * 0.6) + "px");
							$(".ma").text("兑奖码："+data.cc);
							this.$dialog.addClass(this.LOTTERIED_CLS);
							this.$dialog.find('.award-win').removeClass('none');
							this.$dialog.find('.btn').css({"background":"url(./images/try.png) 0 0 no-repeat","background-size":"100%"});
							this.$dialog.find('.contact').removeClass('redbag');
							$(".award-win").css({"background": "url(./images/red-bg.png) no-repeat 100% 100%!important","background-size": "100% 100% !important"})
                            ...
                            this.$dialog.addClass(this.LOTTERIED_CLS);
                            this.$dialog.find('.award-win').removeClass('none');
                            this.$dialog.find('.btn').css({"background":"url(./images/rednew-btn.png) 0 0 no-repeat","background-size":"100%"});
							this.$dialog.find('.contact').removeClass('redbag');
							$(".award-win").css({"background": "url(./images/red-bg.png) no-repeat 100% 100%!important","background-size": "100% 100% !important"})
							...
							var me = H.dialog.lottery;
								...
								me.$dialog.find('.btn').css({"background":"url(./images/rednew-btn.png) 0 0 no-repeat","background-size":"100%"});
								//me.$dialog.find('.btn-share').css({"background":"url(./images/try.png) 100% 100% no-repeat","background-size":"100%"});
								//$(".award-win").css("margin","15% auto");
								$(".award-img").css("margin","40% auto 15%");
								$(".award-win").css({"background": "url(./images/rednew.jpg) no-repeat 100% 100%!important","background-size": "100% 100% !important"})
			...
			check: function() {
				...
				if((H.dialog.lottery.pt == 4)){
					...
					getResult('api/lottery/award', {
						...
						hi: headimgurl ? headimgurl : "",
						oi: openid
					}, 'callbackLotteryAwardHandler', true, H.dialog.$dialog);
				}else{
							...
							._('<div class="content">')
								._('<div class="back">')
									._('<div class="award-win none">')
										...
										._('<div id="awred" class="award-img">')
										._('<img id="aw" src="" />')
										._('<div id="ma" class="ma"></div>')
										._('</div>')
										._('<div class="contact">')
