...
function anictrl(elopen,elcls,object,btnani,ani,startfunc,endfunc,anitime,anidur){
    var isopen = false;
    var $elopen,$elcls,$ob;
...
var add_yao_prefix = function(url) {
    return 'https://yao.qq.com/tv/entry?redirect_uri=' + encodeURIComponent(url);
};
                ...
                if(openid){
                    window['shaketv'] && shaketv.subscribe({
                        ...
                        if (returnData.errorCode == 0) {
                            recordUserOperate(openid, "一键关注_成功", "subscribe_OK");
                        } else if (returnData.errorCode == -1002) {
                            recordUserOperate(openid, "一键关注_用户取消", "subscribe_Cancel");
                        } else if (returnData.errorCode == 9528) {
                            recordUserOperate(openid, "一键关注_用户已经关注", "subscribe_Concerned");
                        }
                    });
                };
            };
