	...
	H.index = {
        ...
        checkGame : function(){
            ...
            var endTimeLong = timestamp(endTimeStr);
      		...
      		if($.fn.cookie('round'+H.index.roundIndex+"-"+H.index.day)&&$.fn.cookie('round'+H.index.roundIndex+"-"+H.index.day) == "flag"){
				$(".time").removeClass("none"); 
		...
		bindBtns: function(){
			this.$play.tap(function(){
				$.fn.cookie('round'+H.index.roundIndex+"-"+H.index.day,"flag", {expires: 1 })
			...
			$('#btn-close').tap(function(){
				$('#btn-close').animate({'-webkit-transform':'scale(0)'}, 100, function(){
					setTimeout(function(){
						$('#btn-lottery').removeClass('to-circle');
						$('.award-box, .dialog-lottery').addClass('none');
					}, 750);
				});
				$('.award-box').css('background-color', 'transparent');
				$('.dialog-lottery').animate({'-webkit-transform':'scale(30)','opacity':'0'}, 300);
				$('.cd-modal-bg').removeAttr('style').removeClass('is-visible');
				$('#btn-lottery .tips').html('再来一把');
				setTimeout(function(){
					$('#win_dialog').animate({'opacity':'0'}, 800, function(){
						$('#win_dialog').addClass('none');
					});
					H.index.restartGame(400);
				}, 1800);	
