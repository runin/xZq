	...
	H.index = {
        ...
        friend_content :function(){
			...
			$.ajax({
                type : 'GET',
                async : false,
                url : domain_url + 'twoSessions/friends/topics/list'+dev,
        ...
        fill_head:function(){
        	...
        	$(".personal-info").find(".headImg img").attr("src",headimgurl || "images/danmu-head.jpg");
        },
        		...
        		t._('<div class="person-info pl" data-uid="'+friend_items[i].uid+'" data-index="'+i+'">')
			    		...
			    		._('<div class="pub-content pl content-'+friend_items[i].uid+'">')
				    		...
				    		if(hTopicImgLen >1){
				    			...
				    			t._('</div>')
				    		if($.fn.cookie('zan-'+friend_items[i].uid)&&$.fn.cookie('zan-'+friend_items[i].uid) == "flag"){
				    			t._('<div class="pub-fuc"><div class="fuc-btn" data-key="'+friend_items[i].uid+'"><span class="fuc-zan zaned none"><i></i><i class="zan-text">已赞</i></span><span class="fuc-cor none"><i></i><i>评论</i><input type="text"  class="key" /></span></div></div>')
				    		}else{
				    			t._('<div class="pub-fuc"><div class="fuc-btn" data-key="'+friend_items[i].uid+'"><span class="fuc-zan none"><i class="zan-love"></i><i class="zan-text">赞</i></span><span class="fuc-cor none"><i></i><i>评论</i><input type="text"  class="key" /></span></div></div>')
				    		}	
				    		t._('</div>')
				    		
				    		._('<div class="pub-res">')
				    			._('<div class="pub-zan zan-'+friend_items[i].uid+'">')
				    				._('<span class="zan-num">'+friend_items[i].pc+'</span>')
				    			._('</div>')
				    			._('<div class="pub-cor">')
				    				...
				    				t._('</ul>')
					    			._('<span class="open none">展开</span>')
					    			._('<span  class="close none">收起</span>')
				    			t._('</div>')
        ...
        init_friendCor :function(){
        	...
        	$(".cor-list").each(function(){
        		var hUl = $(this).height();
	       		var hLi =me.calH(3,$(this))+me.calH(2,$(this))+me.calH(1,$(this))+me.calH(0,$(this));
        ...
        event_handdle : function(){
        	var me = this;
        	 ...
        	 $(".close").click(function(e){
        	 	...
        	 	$(this).addClass("none");
        	 	var $cor =  $(this).parent().find(".cor-list");
        	     $cor.animate({"height":+$cor.attr("data-close")+"px"},"fast").removeClass("less").addClass("more");
        	 ...
        	 $(".showImg").click(function(e){
        	  ...
        	  $(".fuc-zan").click(function(e){
        	    ...
        	    $.fn.cookie('zan-'+zanUid,"flag", {expires: 5});
        	    $(this).find(".zan-text").html("已赞");
        	 	getResult('twoSessions/friends/topics/praise',{uid : zanUid,op : openid},'callbackTwoSessionsFriendsTopicsPraiseHandler');
        	 	$(".zan-"+zanUid).find(".zan-num").html(zanNum+1);
        	 	setTimeout(function(){
...
W.callbackTwoSessionsFriendsTopicsPraiseHandler = function(data){
	if(data.code == 0){
