	...
	H.index = {
		...
		bindBtns: function(){
			...
			$('#btn-lottery').tap(function(){
				...
				if(H.index.wxCheck){
					recordUserOperate(openid, "调用闯关抽奖接口", "doChanllengeLottery");
					$.ajax({
		                type : 'GET',
		                async : false,
		                url : domain_url + 'api/lottery/luck4chanllenge'+dev,
		                data: { oi: openid , sn : sn},
		                dataType : "jsonp",
		                jsonpCallback : 'callbackLotteryLuck4chanllengeHandler',
		                timeout: 10000,
		                complete: function() {
		                	sn = new Date().getTime()+'';
