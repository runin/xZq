		...
		return this.each(function(){
			...
			fileSelected = function() {
			    ...
			    if ($fileUpload.files && $fileUpload.files[0]) {
			        ...
			        t._('<a href="#" id="'+ imgId +'" class="img-preview loading" data-collect="true" data-collect-flag="news-baoliao-preview-btn" data-collect-desc="预览图片按钮">')
						._('<span class="lc"><label class="uploading"></label></span>')
			        ...
			        reader.onload = function (e) {
			        	if (e.target.result && e.target.result.indexOf('image/') !== -1) {
							...
							$(".img-preview").remove();
							$(".popsuccess").removeClass("none").addClass("show");
							setTimeout(function() {
								$(".popsuccess").addClass("none").removeClass("show");
							},5000)
			        	}
			        }
			...
			initForm = function() {
				...
				t._('<form class="'+ opts.formCls +'" enctype="multipart/form-data" method="post">')
					._('<input type="file" accept="image/*" capture="camera" multiple="true" />')
				._('</form>');
