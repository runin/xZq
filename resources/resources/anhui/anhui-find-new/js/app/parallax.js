...
//  'use strict';
    ...
    function init() {
        ...
        pageWidth   = $(".wrapper").width();         	// 获取手机屏幕宽度
        pageHeight  = $(".wrapper").height();       	// 获取手机屏幕高度
        ...
        if (options.arrow) {
        	$pageArr.append('<span class="parallax-prearrow"></span>')
            $pageArr.append('<span class="parallax-arrow"></span>');
            $($pageArr[pageCount-1]).find('.parallax-arrow').remove();
            $($pageArr[0]).find('.parallax-prearrow').remove();
        }
        animatePage(window.curPage);	
    }
