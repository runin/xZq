(function($) {
		...
		tpl: function(data) {
			...
			for (var i = 0, len = item.length; i < len; i++) {
				...
				t._('<li data-uuid = "'+ item[i].uid +'">')
					...
					._('<div>')
						._('<label class="zan '+isZan+'" data-collect="true" data-collect-flag="first-time-comments-zan" data-collect-desc="点赞" >'+ item[i].pc +'</label>')
						._('<p>'+ (item[i].na || '匿名用户') +'</p>')
						._('<p class="all-con" id="all-con'+ me.item_index +'">')
							._('<span>'+ item[i].co +'</span>')
						._('</p>')
						._('<a class="show-all none" id="show-all'+ me.item_index +'" data-collect="true" data-collect-flag="first-time-comments-show" data-collect-desc="评论收缩显示" >^显示全部</a>')
					._('</div>')
					._('</li>');
				++ me.item_index;
			}
	...
	W.fashionIndexNewHandler = function(data){
		if(data.code == 0){
			...
			$("#comm-title span").text(com.actTle);
			...
			if(com.attrs != null && com.attrs.length != 0){
				...
				for (var i = 0, len = result.length; i < len; i++) {
					$sx_ul = $('#sup');
					if(result[i].ai != ""){
						t._('<a class="support" data-collect="true" data-collect-flag="first-time-vote-item" data-collect-desc="投票选项" data-uuid = "'+ result[i].au +'"><i class="hand" style="background: url('+result[i].ai+') no-repeat right center;background-size:17px auto"></i><label>'+result[i].av+'</label></a>');
					}else{
						t._('<a class="support" data-uuid = "'+ result[i].au +'"><i></i><label class="chan-width">'+result[i].av+'</label></a>');
					}
				}
	...
	W.callbackCommentsSave = function(data){
		if(data.code == 0 ){
			...
			._('<img src="'+ headImg +'"/>')
			._('<div>')
				._('<label class="zan-'+data.uid+'" class="zan" data-collect="true" data-collect-flag="first-time-comments-zan" data-collect-desc="点赞" >'+ 0 +'</label>')
				._('<p>'+ (nickname || '匿名用户') +'</p>')
				._('<p class="all-con" id="all-con'+ data.uid +'">')
					._('<span>'+ $("#comments-info").val() +'</span>')
				._('</p>')
				._('<a class="show-all none" id="show-all'+ data.uid +'" data-collect="true" data-collect-flag="first-time-comments-show" data-collect-desc="评论收缩显示">^显示全部</a>')
			._('</div>')
			._('</li>');
