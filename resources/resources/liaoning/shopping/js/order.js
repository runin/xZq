...
(function($) {
	...
	H.order = {
		...
		init: function() {
			...
			getResult('ikea/mall/item/detail/'+ this.uid, {
				yo: openid
			}, 'callbackIkeaMallItemDetail', true);
		},
		...
		submit: function(ischeck) {
			...
			getResult('ikea/mall/order/confirm/'+ this.uid +'/' + this.quantity, {
				yo: openid,
				ph: mobile,
				pt: 1,
				...
				scn: this.scn
			}, 'callbackIkeaMallOrderConfirm', true);
		},
		...
		fill_data: function(data) {
			...
			if(data.esp > 0){
				$dikou_jg.removeClass('none').find('span').text(data.esp);
			}
			...
			if(data.kc <= 0 && data.kc != -1){
				...
				$("#btn-confirm").text("卖光了");
			}
			$("#btn-confirm").removeClass("none");
			...
			$('#mobile').val(data.ph || '');
			$('#name').val(data.man || '');
			$('#address').val((data.pr || '') + (data.ci || '') + (data.di || '') + (data.ad || ''));
		}
	};
	...
	// 确认订单
	W.callbackIkeaMallOrderConfirm = function(data) {
