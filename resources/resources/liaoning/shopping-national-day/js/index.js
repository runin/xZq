...
(function($) {
	...
	H.index = {
		...
		init: function() {
			setTimeout(function(){
				H.index.$lottery_btn.addClass('cjani1');
				setTimeout(function(){H.index.$lottery_btn.removeClass('cjani1').addClass('cjani2')}, 2600);
			}, 50);
		...
		event_handler: function() {
			var me = this;
			H.index.$lottery_btn.click(function(e){
				e.preventDefault();
				if (H.index.clickFlag) {
					H.index.clickFlag = false;
				}else{
					return;
				}
				showLoading();
				$.ajax({
					...
					error : function() {
						H.dialog.shiwuLottery.open(null);
					}
				});
			});
	...
	H.swipe = {
		...
		resize_window: function() {
			...
			if(window.screen.height==480){
				this.$wrapper.css('marginTop', height * 0.008);
				this.$container.css('height', height * 0.9);
			}else{
				this.$wrapper.css('marginTop', height * 0.04);
			...
			for (var i = 0, len = data.length; i < len; i ++) {
				...
				t._('<div class="ui-order-item" data-url="'+ item.hrf +'" data-uid="'+ item.uid +'" data-index="'+ item.in +'" data-ss="'+ item.ss +'" data-collect="true" data-collect-flag="shopping-index-todetail" data-collect-desc="宜佳购物 下单">')
					._('<div class="item-detail">')
						...
						._('<div class="detail-time">');
							if (item.ss == 1) {
								...
								._('<strong>'+ dateformat(str2date(item.vst || ''), 'MM月dd日 hh:mm') +'</strong>');
							} else {
								t._('<label>还剩</label>')
					...
					._('<div class="item-ctrl">');
						...
						var btn_cls = item.ss > 0 ? 'btn-disabled' : '';
